...
def render(H, W, K, ref_K, k_ref, rp_k_ref, coords=None, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
           target_pose=None, ref_poses=None, ref_rgbs=None, it_no=None,
    ...
    # all_ret_sampler = render_rays_sampler(rays, or_rays, coords, target_pose, ref_poses, ref_rgbs,
    #                       H, W, K, ref_K, k_ref, rp_k_ref, **kwargs)
    # all_ret['rgb_map0'] = all_ret_sampler['rgb_map1']
    # all_ret['depth_map'] = all_ret_sampler['depth_map']

    if it_no is not None:
        if it_no % 2 == 0:
            all_ret = render_rays_sampler(rays, or_rays, coords, target_pose, ref_poses, ref_rgbs,
                                  H, W, K, ref_K, k_ref, rp_k_ref, **kwargs)
        else:
            all_ret = render_rays(rays, or_rays, coords, target_pose, ref_poses, ref_rgbs,
                                  H, W, K, ref_K, k_ref, rp_k_ref, **kwargs)
...
def render_path(render_poses, hwf, K, k_ref, rp_k_ref, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    return rgbs0, rgbs1, depths, mean_warps, rgbs_n0
...
def create_nerf(args):
    ...
    model_mmray = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                      ...
                                      output_ch=3 * args.N_samples + 3, skips=args.mmnetskips).to(device)
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    if k_ref > 0:
        raw = network_query_fn(torch.cat((pts.detach(), warps_flat.detach()), 2), viewdirs, network_fn)
    else:
        raw = network_query_fn(pts.detach(), viewdirs, network_fn)

    # Shift and scale in densities
    raw = torch.cat((raw[:, :, :3], (raw[:, :, 3] + z_densities_a.detach()).unsqueeze(2)), -1)
    rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std, white_bkgd,
                                                                 depth_densities=z_densities_b.detach(), pytest=pytest)

    ret = {'rgb_map0': rgb0.detach(), 'rgb_map1': rgb_map, 'depth_map':mean_epi_depth.detach(), 'mean_warp':depth_map,
           'rgb_map_n0': torch.abs(rgb0 - rgb_map).detach()}
...
def render_rays_sampler(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
                k_ref, rp_k_ref,
                network_fn,
                network_query_fn,
                N_samples,
    ...
    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb_map, 'depth_map':mean_epi_depth, 'mean_warp':depth_map,
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs0, rgbs1, depths, mean_warps, rgbsn0 = r_out[0], r_out[1], r_out[2], r_out[3], r_out[4]
    ...
    for i in trange(start, N_iters):
        ...
        rgb0, rgb1, depth, mean_warp, extras = render(H, W, K, ref_K, args.k_ref, args.rp_k_ref,
                                                      target_pose=pose, ref_poses=ref_poses, it_no=i,
                                                        ref_rgbs=ref_rgbs,
        ...
        loss = img_loss + args.a_mmrgb * img2mse(rgb0, target_s)
