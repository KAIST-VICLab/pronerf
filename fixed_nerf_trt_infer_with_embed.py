...
from trt_infer_with_embed import *
...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024*64):
    ...
    outputs_flat = fn(inputs_flat, input_dirs_flat)
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0, near=0., far=1., or_near=1., or_far=10.):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if render_kwargs['use_trt']:
            ...
            render_kwargs['nerf_engine'].bind_input_dir(input_dirs_flat.cpu().numpy()) # bind viewdirs to nerf

            input_holder = torch.zeros(input_dirs_flat.shape[0], 3).flatten().cuda() # bind dummy input xyz to nerf
...
def create_nerf(args):
    ...
    skips = [4]
    model = NeRFEmbedTRT(D=args.netdepth, W=args.netwidth,
                 input_ch=input_ch, output_ch=output_ch, skips=skips,
                 input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs, multires=args.multires, multires_dir=args.multires_views)
    if not args.use_trt:
        model.to(device)

    model_fine = None
    model_fine = NeRFEmbedTRT(D=args.netdepth_fine, W=args.netwidth_fine,
                        input_ch=input_ch, output_ch=output_ch, skips=skips,
                        input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs, multires=args.multires, multires_dir=args.multires_views)
    if not args.use_trt:
    ...
    # export to onnx
    # model2onnx(model_fine, in_ch = [3, 3], savedir = os.path.join(args.basedir, args.expname), model_name='nerf', batch_size=1)
...
def render_rays(ray_batch, or_ray_batch,
    ...
    if kwargs['use_trt']:
        t1.record()
        flat_query_points = query_points_nerf.view(-1, 3).flatten()
        # embed_xyz = embed_fn(flat_query_points).flatten()
        kwargs['nerf_engine'].bind_input(flat_query_points) # bind query points to nerf
