...
def create_nerf(args):
    ...
    skips = args.netskips
    if args.k_ref > 0:
        model = NeRF_epi2(D=args.netdepth, W=args.netwidth,
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    if raw0 is not None:
        raw = torch.cat((raw0[:, :, :3], (raw0[:, :, 3] + z_densities_a).unsqueeze(2)), -1)
        rgb_map00, _, _, _, _ = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std, white_bkgd,
                                                                     depth_densities=z_densities_b, pytest=pytest)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if 'rgb_map_n0' in extras.keys():
            loss = loss + img2mse(extras['rgb_map_n0'], target_s)
