...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    # Render and reshape
    all_ret = render_rays(rays, **kwargs)
    for k in all_ret:
...
def render_rays(ray_batch,
    ...
    mm_rgb = torch.sigmoid(min_max_rays[:, 4*N_samples:])
    mm_density_add = min_max_rays[:, 2*N_samples:3*N_samples]
    mm_density_mul = min_max_rays[:, 3*N_samples:4*N_samples]
    ...
    if randomize:
        ...
        depth_values = depth_values_mean + noise_step*depth_values_var
        depth_values[depth_values<0] = 0
        depth_values[depth_values>1] = 1
    else:
        depth_values = depth_values_mean
