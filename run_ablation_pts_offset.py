...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        max_offsets.append(extras['max_offset'].cpu().numpy())
    ...
    max_offsets = np.stack(max_offsets,0)
    if len(psnrs) > 0:
        mean_psnr = 0
...
def render_rays(ray_batch, or_ray_batch,
    ...
    invalid_index = torch.abs(points_offset[...,0]) > 0.5
    points_offset[...,0][invalid_index] = torch.sign(points_offset[...,0][invalid_index])*0.0

    # if torch.any(invalid_index):
    #     print('Large offset x!')

    invalid_index = torch.abs(points_offset[...,1]) > 0.5
    points_offset[...,1][invalid_index] = torch.sign(points_offset[...,1][invalid_index])*0.0

    # if torch.any(invalid_index):
    #     print('Large offset y!')

    invalid_index = torch.abs(points_offset[...,2]) > 0.5
    points_offset[...,2][invalid_index] = torch.sign(points_offset[...,2][invalid_index])*0.0

    # if torch.any(invalid_index):
    #     print('Large offset z!')

    query_points_nerf = query_points_nerf + (1e-2) * points_offset
    max_offset, _ = torch.max(torch.abs(points_offset).detach().view(-1,3), dim =0)
    ...
    ret = {'rgb_map0': refine_rgb, 'rgb_map1': rgb_map, 'depth_map': depth_map, 'mm_rgb': mm_rgb, 'z_vals': epi_z_vals.mean(dim=-1), 'z_vals0': depth_values.mean(dim=-1), 'max_offset': max_offset[None].repeat(N_rays,1)}
    return ret
