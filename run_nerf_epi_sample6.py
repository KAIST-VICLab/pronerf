...
from loss_functions import perceptual_loss, vgg, smoothness
import inverse_warp
...
def config_parser():
    ...
    parser.add_argument("--a_dsm", type=float, default=0,
                        help='weight for depth smoothness loss')
...
def create_nerf(args):
    ...
    model_refine = MinMaxRayS1Conv_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                  ...
                                  3 + (3 + args.k_ref * 3) * args.N_samples + 3 * args.N_point_ray_enc,
                                  output_ch=args.N_samples + 3, skips=args.mmnetskips)
    grad_vars.append({'params': model_refine.parameters(), 'weight_decay': args.weight_decay, 'lr': args.lrate})

    model_refine1 = MinMaxRayS1Conv_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                  input_ch=args.k_ref * 3 + input_ch * args.N_point_ray_enc if args.mm_emb else
                                  3 + (3 + args.k_ref * 3) * args.N_samples + 3 * args.N_point_ray_enc,
                                  output_ch=args.N_samples + 3, skips=args.mmnetskips)
    grad_vars.append({'params': model_refine1.parameters(), 'weight_decay': args.weight_decay, 'lr': args.lrate})
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, H, W, K, k_ref,
    ...
    if embed_fn is not None:
        pts = embed_fn(pts)
    mm_input0 = torch.permute(pts.view(1, H, W, -1), (0, 3, 1, 2))

    mm_input = F.interpolate(mm_input0, size=(H//2, W//2), mode='bilinear', align_corners=True)
    min_max_rays = min_max_ray_net(mm_input)
    min_max_rays = F.interpolate(min_max_rays, size=(H, W), mode='bilinear', align_corners=True)
    min_max_rays = torch.permute(min_max_rays.view(N_samples + 3, H*W), (1, 0))

    # Depth levels
    epi_z_vals = torch.sigmoid(min_max_rays[:, 0:N_samples]) * (or_far - or_near) + or_near
    ...
    warps = inverse_warp.inverse_warp_rod1_rt2(ref_rgb, depths, ro1, rd1, ref_pose, K, inv_K,
                                               scale=1.0, padding_mode='zeros')
    ...
    mm_input = torch.cat((mm_input0, warps_flat.view(1, k_ref*N_samples * 3, H, W),
                          torch.permute(epi_pts.view(1, H, W, N_samples * 3), (0, 3, 1, 2)),
                          torch.permute(rgb0.view(1, H, W, 3), (0, 3, 1, 2))
                          ), 1)
    mm_input = F.interpolate(mm_input, size=(H//2, W//2), mode='bilinear', align_corners=True)
    min_max_rays = refine_net(mm_input)
    min_max_rays = F.interpolate(min_max_rays, size=(H, W), mode='bilinear', align_corners=True)
    min_max_rays = torch.permute(min_max_rays.view(N_samples + 3, H*W), (1, 0))

    # New Depth levels
    z_vals1 = torch.sigmoid(min_max_rays[:, 0:N_samples]) * (far - near) + near
    z_vals1, _ = torch.sort(z_vals1, dim=-1)
    mean_epi_depth = torch.mean(z_vals1, 1, True)
    if randomize is True:
        noise_shape = list(z_vals1.shape)
        noise_ = (1/6) * torch.normal(0.0, 1.0, size=noise_shape).to(z_vals1)
        noise_ = noise_ * (far - near) / N_samples
        z_vals1 = noise_ + z_vals1
        z_vals1[z_vals1 < 0] = 0
        z_vals1, _ = torch.sort(z_vals1, dim=-1)
    pts = rays_o[..., None, :] + rays_d[..., None, :] * z_vals1[..., :, None]

    # RGB prior
    rgb1 = min_max_rays[:, N_samples::]

    # epi_pts1 = epi_pts1.reshape(-1, N_samples * 3)
    # if embed_fn is not None:
    #     epi_pts1 = embed_fn(epi_pts1)
    #
    # depths = epi_z_vals.view(1, H, W, N_samples).repeat(k_ref, 1, 1, 1)  # k_ref, H, W, N_point_ray_enc
    # depths = torch.permute(depths, (0, 3, 1, 2)).reshape(-1, H, W)  # k_ref * N_point_ray_enc, H, W
    #
    ...
    # invalid_warp = (torch.sum(warps.detach(), 1, True) == 0).type_as(warps)
    # warps = warps * (1 - invalid_warp) - invalid_warp  # make invalid regions -1
    # warps_flat1 = warps.clone().view(1, k_ref, N_samples, 3, H, W)
    #
    # mm_input = torch.cat((mm_input0, warps_flat1.view(1, k_ref * N_samples * 3, H, W),
    #                       torch.permute(epi_pts1.view(1, H, W, N_samples * 3), (0, 3, 1, 2)),
    #                       torch.permute(rgb1.view(1, H, W, 3), (0, 3, 1, 2))
    #                       ), 1)

    #     raw = run_network(pts)
    raw = network_query_fn(pts, viewdirs, network_fn)
    # raw = torch.cat((raw[:, :, :3], (z_densities + raw[:, :, 3]).unsqueeze(2)), -1)
    rgb2, disp_map, acc_map, weights, depth_map = raw2outputs(raw, z_vals1, rays_d, raw_noise_std, white_bkgd,
                                                                 pytest=pytest)

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb2, 'depth_map':depth_map, 'mean_warp':rgb1}
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if args.a_p > 0 and i >= a_sch:
            ...
            rgb_predicted_ = torch.permute(mean_warp.view(th, tw, 3).clone(), (2, 0, 1)).unsqueeze(0)
        ...
        smooth_loss = 0
        if args.a_dsm > 0 and i > a_sch:
            target_img_ = torch.permute(target_s.view(th, tw, 3).clone(), (2, 0, 1)).unsqueeze(0)
            smooth_loss = args.a_dsm * smoothness(target_img_, 1/(depth.view(1, 1, th, tw) + 1e-6))

        loss = img_loss + perc_loss + smooth_loss + img2mse(rgb0, target_s) + img2mse(mean_warp, target_s)
        ...
        if i % 500 == 0:
            ...
            if args.k_ref > 0 and 'warp_0' in extras.keys():
        ...
        if i % args.i_weights == 0:
            ...
            if args.N_importance > 0:
                torch.save({
                    ...
                    'refine_net_state_dict1': render_kwargs_train['refine_net1'].state_dict(),
