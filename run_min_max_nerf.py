...
def render(H, W, K, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
    ...
    k_extract = ['rgb_map', 'disp_map', 'acc_map', 'min_ray', 'max_ray', 'dst']
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    if 'mm_rgb' in extras.keys():
        mm_rgbs = np.stack(mm_rgbs, 0)
        return rgbs, disps, mins, maxs, dsts, mm_rgbs
    else:
        return rgbs, disps, mins, maxs, dsts
...
def create_nerf(args):
    ...
    model_mmray = MinMaxRayS_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                ...
                                output_ch=3 if args.a_mmrgb == 0 else 6, skips=args.mmnetskips)
...
def compute_query_points_from_rays(
  ...
  """
    ...
    # if randomize: #24.84 with random test / 24.833
    ...
    dst = torch.sigmoid(min_max_rays[:, 2, None]) * (10 - 0.1) + 0.1

    mm_rgb = None
    if min_max_rays.shape[1] > 3:
        mm_rgb = min_max_rays[:, 3::]

    if randomize:
        num_samples = int(random.uniform(num_samples // 2, num_samples * 2))
    ...
    if randomize is True:
        ...
        # noise_ = (torch.rand(noise_shape).to(ray_origins) - 0.5) * (max_rays.detach() - min_rays.detach()) / num_samples
        noise_ = (1/5) * torch.normal(0.0, 1.0, size=noise_shape).to(ray_origins)
        # noise_[noise_ > 0.4999] = 0.4999
        # noise_[noise_ < -0.4999] = -0.4999
        noise_ = noise_ * (max_rays.detach() - min_rays.detach()) / num_samples

        # if random.random() > 0.5:
        #     noise_[..., 1:-1] = 0 # only last and first samples have noise
        # else:
        #     noise_[..., 0] = 0 # only last and first samples dont have noise
        #     noise_[..., -1] = 0 # only last and first samples dont have noise
        # if random.random() > 0.5:
        #     noise_ = 0 * noise_
        depth_values = noise_ + depth_values
        depth_values[depth_values < 0] = 0
        depth_values = torch.sort(depth_values, dim=-1)[0]
...
def render_rays(ray_batch,
    ...
    ret = {'rgb_map': rgb_map, 'disp_map': disp_map, 'acc_map': acc_map,
           'min_ray':min_ray, 'max_ray':max_ray, 'dst':dst, 'depth_map':depth_map}
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if args.disp_smoooth > 0:
            n_batch_rays = batch_rays.clone()
            n_batch_rays[1, :, :] = n_batch_rays[1, :, :] + ray_dir_diff.unsqueeze(0)
            _, disp1, _, _, _, _, _ = render(H, W, K, chunk=args.chunk, rays=n_batch_rays, verbose=i < 10, retraw=True,
                                            **render_kwargs_train)
            disp_sm_loss = args.disp_smoooth * torch.mean((disp1 - disp) ** 2)
        ...
        if 'mm_rgb' in extras and args.a_mmrgb > 0:
            mm_rgb_loss = args.a_mmrgb * img2mse(extras['mm_rgb'], target_s)

        mm_disp_loss = 0
        if args.a_mmdisp > 0 and i > 1000:
            mm_disp_loss = args.a_mmdisp * img2mse(extras['depth_map'].detach(), (min_ray + max_ray) / 2)
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            with torch.no_grad():
                ...
                rgbs, disps, mins, maxs, dsts = r_out[0], r_out[1], r_out[2], r_out[3], r_out[4]
