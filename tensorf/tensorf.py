...
class TensorBase(torch.nn.Module):
    ...
    def update_stepSize(self, gridSize):
        ...
        self.register_buffer('gridSize', torch.LongTensor(gridSize).to(self.device))
...
class TensorVMNoSample(TensorVMSplit):
    ...
    def forward(self, x: Dict[str, torch.Tensor]):
        ...
        # Viewdirs
        viewdirs = x["viewdirs"][:,None,:].expand(-1, xyz_sampled.shape[1], -1)
        ...
        if ray_valid.any():
            ...
            # Convert to density
            sigma_feature = sigma_feature.view(sigma_feature.shape[0])
            valid_sigma = self.feature2density(sigma_feature)
