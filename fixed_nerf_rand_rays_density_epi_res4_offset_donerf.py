...
def render_rays(ray_batch, or_ray_batch,
    ...
    plucker_pts = kwargs['embed_rays'](pts, rays_d[:,None,:].expand(-1,N_point_ray_enc,-1)) # nump_pts + origin
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if (i % args.i_testset == 0 and i > 0) or (args.render_test):
            ...
            with torch.no_grad():
                render_path(torch.Tensor(poses[i_val]).to(device), hwf, K, args.chunk, render_kwargs_test, gt_imgs=images[i_val], savedir=testsavedir)
