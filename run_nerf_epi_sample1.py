...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, H, W, K, k_ref,
    ...
    warps_flat = torch.mean(warps_flat, 1)
    # warps_flat = torch.sum(warps_flat, 1) / (torch.sum(1-invalid_warp.view(1, k_ref, N_samples, 1, H, W), 1) + 1e-6)

    mm_input = torch.cat((warps_flat.view(1, N_samples * 3, H, W),
                          torch.permute(epi_pts.view(1, H, W, N_samples * 3), (0, 3, 1, 2))), 1)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if i % 500 == 0:
            ...
            if args.k_ref > 0:
                for num_ref in range((rays_rgb.shape[0]-1) * args.N_samples):
