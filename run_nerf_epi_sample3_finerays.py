...
def render(H, W, K, ref_K, k_ref, coords=None, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
...
def render_path(render_poses, hwf, K, k_ref, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        # print(i, time.time() - t)
        t = time.time()
        rgb0, rgb1, depth, mean_warp, extras = render(H, W, K, torch.Tensor(K.copy()).to(device), k_ref,
...
def create_nerf(args):
    ...
    #              input_ch=input_ch, output_ch=output_ch, skips=skips, input_ch_epi=args.k_ref * 3,
    #              input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
    model = NeRF(D=args.netdepth, W=args.netwidth,
                 input_ch=input_ch+args.k_ref * 3, output_ch=output_ch, skips=skips,
                 input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs).to(device)
    ...
    model_refine = MinMaxRayS1_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                  ...
                                  (3 + args.k_ref*3) * args.N_samples + 3 * args.N_point_ray_enc,
                                  output_ch=3 * args.N_samples + 3, skips=args.mmnetskips).to(device)
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K, k_ref,
                network_fn,
                network_query_fn,
                N_samples,
                retraw=False,
    ...
    z_densities_b = min_max_rays[:, N_samples*2:N_samples*3]
    ...
    if randomize is True:
        ...
        # noise_ = torch.rand(noise_shape).to(epi_z_vals)
        noise_ = (1 / 7) * torch.normal(0.0, 1.0, size=noise_shape).type_as(epi_z_vals)
        noise_ = torch.abs(noise_)
        noise_[noise_ >= 1] = 1 - 1e-6
        if random.random() > 0.5:
            epi_z_vals_diff = torch.abs(
                epi_z_vals - torch.cat((epi_z_vals[:, 1:], or_far * torch.ones(N_rays, 1).type_as(epi_z_vals)), 1))
            noise_ = noise_ * epi_z_vals_diff
        else:
            epi_z_vals_diff = torch.abs(
                epi_z_vals - torch.cat((or_near * torch.ones(N_rays, 1), epi_z_vals[:, 0:-1].type_as(epi_z_vals)), 1))
            noise_ = -noise_ * epi_z_vals_diff
    ...
    depths = epi_z_vals.view(1, 1, N_rays, N_samples).repeat(k_ref, 1, 1, 1) # k_ref, H, W, N_point_ray_enc
    ...
    refine_input = \
        torch.cat((pts, epi_pts.view(N_rays, N_samples * 3), warps_flat.view(N_rays, N_samples * 3 * k_ref)), -1)
    refine_outs = refine_net(refine_input)

    # Aux RGB and Depth levels
    epi_z_vals = torch.sigmoid(refine_outs[:, 0:N_samples]) * (or_far - or_near) + or_near
    z_densities_a = refine_outs[:, N_samples:N_samples*2]
    z_densities_b = refine_outs[:, N_samples*2:N_samples*3]
    rgb1 = refine_outs[:, N_samples*3::]
    ...
    raw = torch.cat((raw[:, :, :3], (raw[:, :, 3] + z_densities_b).unsqueeze(2)), -1)
    rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, z_vals, rays_d, raw_noise_std, white_bkgd,
                                                                 pytest=pytest)

    ret = {'rgb_map0': rgb1, 'rgb_map1': rgb_map, 'depth_map':mean_epi_depth, 'mean_warp':depth_map,
           'rgb_map_00':rgb0, 'acc_map':acc_map}
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        rgb0, rgb1, depth, mean_warp, extras = render(H, W, K, ref_K, args.k_ref, target_pose=pose, ref_poses=ref_poses,
        ...
        loss = img_loss + perc_loss + img2mse(rgb0, target_s) + acc_reg + img2mse(extras['rgb_map_00'], target_s)
