...
def create_nerf(args):
    ...
    grad_vars.append({'params': model_density.parameters(), 'weight_decay': args.weight_decay, 'lr':  args.ngp_lrate})
    ...
    grad_vars.append({'params': model_color.parameters(), 'weight_decay': args.weight_decay, 'lr':  args.ngp_lrate})
...
def render_rays(ray_batch, or_ray_batch, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    upper = torch.cat([mids, 0.5 * (far + depth_values[..., -1:])], -1) - EPS # upper cat far
    lower = torch.cat([0.5 * (near + depth_values[..., :1]), mids], -1)  + EPS # lower cat near
    ...
    if randomize and not train_sampler:
        ...
        rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)        
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if i % args.i_print == 0:
            if psnr_n != 0:
                psnr_n = psnr_n.item()
            tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR_n: {psnr_n}")
