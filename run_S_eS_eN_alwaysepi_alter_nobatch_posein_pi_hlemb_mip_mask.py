...
def render_path(render_poses, hwf, K, k_ref, N_n, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        t = time.time()
        rgb0, rgb1, extras = render(H, W, K, torch.Tensor(K.copy()).to(device), k_ref, N_n,
...
def create_nerf(args):
    ...
    embed_fn, input_ch = get_embedder(5, args.i_embed)
    embed_rays = Pluecker()
    ...
    model_mmray = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                input_ch=2 + (input_ch-3) * args.N_point_ray_enc
                                if args.mm_emb else 2 + args.N_point_ray_enc*9,
...
def render_rays(ray_batch, or_ray_batch, p_uv, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    # m = m.view(N_rays, N_point_ray_enc, -1)[...,3:]

    # m = m.reshape(N_rays, -1)
    ...
    mm_alpha = torch.sigmoid(min_max_rays)
    ...
    # mm_alpha = torch.sigmoid(min_max_rays[:, 3 * N_samples+3:])
    ...
    ret = {'rgb_map0': mm_alpha, 'rgb_map1': mm_alpha, 'depth_map': mm_alpha, 
           'mm_rgb': mm_alpha, 'mm_alpha':mm_alpha}
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        render_kwargs_train['train_nerf'] = True
        render_kwargs_train['train_sampler'] = True
        rgb0, rgb1, extras = render(H, W, K, ref_K, args.k_ref, args.N_n,
        ...
        loss = F.binary_cross_entropy(extras['mm_alpha'], a_chann[:,None])
        # mask_loss = 0
        # loss = img2mse(extras['mm_rgb'], target_s) + 0.0*mask_loss
        # psnr_sn = mse2psnr(img2mse(extras['mm_rgb'].detach(), target_s))
        loss.backward()
        s_optimizer.step()
