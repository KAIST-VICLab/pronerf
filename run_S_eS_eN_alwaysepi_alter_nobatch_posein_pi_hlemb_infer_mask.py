...
def render(rays, or_rays, sh, **kwargs):
    ...
    # mask infer to prune
    min_max_rays = kwargs['mask_net'](kwargs['mask_input'])
    mm_alpha = torch.sigmoid(min_max_rays)
    mm_alpha = mm_alpha.view(1,1,kwargs['Hfull'], kwargs['Wfull'])
    mm_alpha = F.max_pool2d(mm_alpha, kernel_size=11, padding=5, stride=1)
    mm_alpha = mm_alpha.view(-1, 1)
    # render_index = mm_alpha > 0.4
    _, render_index = torch.topk(mm_alpha, k = int(800*800*kwargs['compress_rate']), dim = 0)
    ...
    for i in range(0, rays.shape[0], chunk):
        ...
        kwargs['rays_radii'] = rays_radii[i:i + chunk]
        kwargs['render_index'] = render_index[i:i + chunk]
    ...
    ret_dict['compress_rate'] = kwargs['compress_rate']
...
def render_path(render_poses, hwf, K, k_ref, N_n, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        # mask input
        plucker_pts = render_kwargs['embed_rays'](pts, rays_d[:, None, :].repeat(1, render_kwargs['N_point_ray_enc'], 1))
        plucker_pts = torch.cat([plucker_pts, pts], dim =-1) 
        plucker_pts = plucker_pts.view(-1, (render_kwargs['N_point_ray_enc']) * 9)
        render_kwargs['mask_input'] = torch.cat((plucker_pts, p_uv), -1) # + puv
        ...
        rgb1, extras = render(rays, or_rays, sh, **render_kwargs)

        # rgbs0.append(rgb0.cpu().numpy())
        rgbs1.append(rgb1.cpu().numpy())
        compress_rates.append(extras['compress_rate'])
    ...
    print('Avg rate:', round(np.array(compress_rates).mean(),3))

    return rgbs1, rgbs0, depths, nerf_depths, rgbs_n0
...
def create_nerf(args):
    ...
    model_mask = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
...
def render_rays(ray_batch, or_ray_batch, p_uv, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, ref_K,
    ...
    shader_index = kwargs['render_index'].squeeze()
    shader_mm_input = kwargs['mm_input'][shader_index]
    shader_N_rays = shader_mm_input.shape[0]
    ...
    # This code no grad???
    depths = depth_values.view(1, 1, shader_N_rays, N_point_ray_enc).expand(N_n, -1, -1, -1)  # k_ref, H, W, N_point_ray_enc
    depths = torch.permute(depths, (0, 3, 1, 2)).reshape(-1, 1, shader_N_rays)  # k_ref * N_point_ray_enc, H, W
    ...
    warps_flat = warps.clone().view(1, N_n, N_point_ray_enc, C_color, 1, shader_N_rays)

    warps_flat = torch.permute(warps_flat.view(N_n, N_point_ray_enc, C_color, shader_N_rays),
                               (3, 1, 0, 2)).reshape(shader_N_rays, N_point_ray_enc, C_color * N_n)
    # This code^ no grad???
    ...
    warps_flat_1 = warps_flat.view(shader_N_rays, N_samples, -1)
    input_ref_poses = kwargs['ref_pose_in'][shader_index]
    # input_ref_poses = ref_poses[ref_nos, :, 3].view(1, N_n * 3).expand(N_rays, N_n * 3)
    net_input = torch.cat((m.view(shader_N_rays, -1), warps_flat_1.view(shader_N_rays, -1), shader_p_uv, input_ref_poses), -1)
    ...
    # add point offset
    query_points_nerf = shader_rays_o[..., None, :] + shader_rays_d[..., None, :] * refine_depth_values[..., :, None]
    if train_sampler:
    ...
    depths = refine_depth_values.view(1, 1, shader_N_rays, N_point_ray_enc).expand(N_n0, -1, -1, -1)  # k_ref, H, W, N_point_ray_enc
    depths = torch.permute(depths, (0, 3, 1, 2)).reshape(-1, 1, shader_N_rays)  # k_ref * N_point_ray_enc, H, W

    warps, _ = inverse_warp.inverse_warp_rod1_rt2_coords_trt(ref_rgb, depths, ro1, rd1, ref_pose, padding_mode='zeros')
    
    warps_flat = warps.clone().view(1, N_n0, N_point_ray_enc, C_color, 1, shader_N_rays)
    warps_flat = torch.permute(warps_flat.view(N_n0, N_point_ray_enc, C_color, shader_N_rays),
                               (3, 1, 0, 2)).reshape(shader_N_rays, N_point_ray_enc, C_color * N_n0)

    input_ref_posesn = kwargs['ref_pose_in'][shader_index]
    input_ref_posesn = input_ref_posesn[:, None].expand(shader_N_rays, N_samples, N_n * 3)
    p_uv_n = shader_p_uv.view(shader_N_rays, 1, 2).expand(shader_N_rays, N_samples, 2)

    if kwargs['use_trt']:
        inputs = torch.cat((query_points_nerf, warps_flat.view(shader_N_rays, N_samples, -1), input_ref_posesn, p_uv_n), -1)
        inputs_flat = torch.reshape(inputs, [-1, inputs.shape[-1]])
        embedded = embed_fn(inputs_flat[:, 0:3])
        embedded = torch.cat((embedded, inputs_flat[:, 3::]), -1)
        input_dirs = shader_viewdirs[:, None].repeat(1, inputs.shape[1], 1)  # .expand(inputs.shape)
        input_dirs_flat = torch.reshape(input_dirs, [-1, input_dirs.shape[-1]])
        embedded_dirs = embeddirs_fn(input_dirs_flat)
        embedded = torch.cat([embedded, embedded_dirs], -1)
        ...
        raw = network_query_fn(
            torch.cat((query_points_nerf, warps_flat.view(shader_N_rays, N_samples, -1), input_ref_posesn, p_uv_n), -1),
            shader_viewdirs, network_fn)

    raw_noise_std = 0
    shader_rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, refine_depth_values, shader_rays_d, raw_noise_std,
