...
def create_nerf(args):
    ...
    model_mmray = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                      ...
                                      output_ch=5 * args.N_samples + 3, skips=args.mmnetskips).to(device)
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    z_densities_b = min_max_rays[:, N_samples*2:N_samples*5]
    z_densities_b = z_densities_b.view(N_rays, -1, 3)
    rgb0 = min_max_rays[:, N_samples*5::]
    ...
    # sort_indices = sort_indices[:, :, None].repeat(1, 1, 3)
    # z_densities_b = z_densities_b.gather(dim=1, index=sort_indices.detach())

    raw = torch.cat((z_densities_b, z_densities_a.unsqueeze(2)), -1)
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            with torch.no_grad():
                render_path(torch.Tensor(poses[i_test]).to(device), hwf, K, args.k_ref, args.rp_k_ref, args.chunk,
                            ref_poses=poses_train, ref_rgbs=rays_rgb[:, :, :, 2, :], render_kwargs=render_kwargs_test,
