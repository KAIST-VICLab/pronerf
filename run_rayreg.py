...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    # pts = pts.view(N_rays, N_point_ray_enc, 3)
    # m_dirs = rays_d.clone().view(N_rays, 1, 3).repeat(1, N_point_ray_enc, 1)
    # m = torch.cross(pts.view(-1, 3), m_dirs.view(-1, 3), dim=-1)
    # m = torch.cat([rays_d, m.view(N_rays, N_point_ray_enc*3)], dim=-1)
    # m = m.view(N_rays, -1)
    ...
    if rp_k_ref > 0:
        with torch.no_grad():
            if randomize:
                # Random
                # ref_nos = random.sample(range(ref_rgbs.shape[0]), k=rp_k_ref)
                ...
                # First k_ref lines
                # ref_nos = range(ref_rgbs.shape[0])[0:rp_k_ref]
            ...
            # if randomize:
            #     mask = (torch.rand((1, rp_k_ref, N_point_ray_enc, 1, 1, N_rays)) > 0.5).type_as(warps)
            #     warps_flat = mask.detach() * warps_flat
        ...
        rgb0 = min_max_ray_net(pts, warps_flat)
    else:
        rgb0 = min_max_ray_net(pts)
        rgb1 = rgb0

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb0, 'depth_map':rgb0[:, 0, None], 'nerf_depth':rgb0[:, 0, None],
           'rgb_map_n0': torch.abs(rgb0 - rgb0)}
