...
def render(H, h, W, w, K, ref_K, k_ref, coords=None, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
    ...
    # Render and reshape
    all_ret = render_rays(rays, or_rays, coords, target_pose, ref_poses, ref_rgbs, H, h, W, w, ref_K, k_ref, **kwargs)
    for k in all_ret:
...
def create_nerf(args):
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        model_refine1.load_state_dict(ckpt['refine_net_state_dict1'])
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, H, Wfull, W, K, k_ref,
                network_fn,
                network_query_fn,
                N_samples,
                retraw=False,
    ...
    warps = inverse_warp.inverse_warp_rod1_rt2_coords(ref_rgb, depths, ro1, rd1, ref_pose, K, inv_K,
    ...
    # New Depth levels
    epi_z_vals1 = torch.sigmoid(min_max_rays[:, 0:N_samples]) * (or_far - or_near) + or_near
    epi_z_vals1, _ = torch.sort(epi_z_vals1, dim=-1)
    mean_epi_depth = torch.mean(epi_z_vals, 1, True)
    epi_pts1 = or_rays_o[..., None, :] + or_rays_d[..., None, :] * epi_z_vals1[..., :, None]
    # RGB prior
    ...
    mm_input = torch.cat((warps_flat1.view(1, k_ref * N_samples * 3, H, W),
                          torch.permute(epi_pts1.view(1, H, W, N_samples * 3), (0, 3, 1, 2)),
                          torch.permute(rgb1.view(1, H, W, 3), (0, 3, 1, 2))
                          ), 1)
    min_max_rays = refine_net1(mm_input)
    min_max_rays = torch.permute(min_max_rays.view(N_samples + 3, H * W), (1, 0))

    # Final
    rgb2 = min_max_rays[:, N_samples::]

    #  Vol rendering
    # raw = torch.cat((z_rgbs, z_densities.unsqueeze(2)), -1)
    # rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, or_rays_d, raw_noise_std, white_bkgd,
    #                                                              pytest=pytest)
    # ret = {'rgb_map0': rgb0, 'rgb_map1': rgb_map, 'depth_map':depth_map, 'mean_warp':mean_rgb}

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb2, 'depth_map':mean_epi_depth, 'mean_warp':rgb1}
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        ref_rgbs = ref_rgbs.view(-1, H*W, 3) # (N_ref, N_rand, 3)
