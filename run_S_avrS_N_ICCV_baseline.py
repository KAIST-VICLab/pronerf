...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, default='configs/fern/fern_baseline.txt',
...
def create_nerf(args):
    ...
    if args.epi_nerf:
        ...
        # model = DoNeRF_var(D=args.netdepth, W=args.netwidth,
        #                n_in=input_ch + input_ch_views, n_out=output_ch, skip='auto')
    model.to(device)
    ...
    model_refine = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                 ...
                                 output_ch=4 * args.N_samples + args.num_neighbor * args.N_samples + 3,
...
def render_rays(ray_batch, or_ray_batch,
    ...
    refine_rgb = torch.sum(weights.view(-1, N_samples * num_neighbor, 1) *
                           epi_features.view(N_rays, N_samples * num_neighbor, 3), dim=1) + refine_output[:, -3:]
    ...
    if train_sampler:  # and iter >= 30000:
        ...
        # Baseline (ICCV) Mean Test PSNR 24.35
        # fern_sampler_avrRGB1_e2e_donerf_8samples # 24.328

        # with torch.no_grad(): # 23.76
        #     rot_mat = inverse_warp.rotation_matrix_from_vectors(viewdirs, rays_d)
        #     rot_mat = rot_mat[:, None, :, :].expand(-1, N_samples, -1, -1)
        # points_offset = torch.bmm(rot_mat.reshape(N_rays * N_samples, 3, 3),
        #                           (1e-2) * points_offset.view(N_rays*N_samples, 3, 1))
        # query_points_nerf = query_points_nerf + points_offset.view(N_rays, N_samples, 3)
    ...
    ret = {'rgb_map0': refine_rgb, 'rgb_map1': rgb_map, 'depth_map': depth_map, 'mm_rgb': mm_rgb,
           'depth_map0': refine_depth_values.mean(dim=-1)}
    if train_sampler:
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if False: #(i % args.i_video == 0 and i > 0) or (args.render_only):
