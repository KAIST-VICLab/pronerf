...
def config_parser():
    ...
    parser.add_argument("--k_ref2", type=int, default=16,
                        help='number reference epipolar lines')
...
def create_nerf(args):
    ...
    model_mmray = MinMaxRayS1Conv_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                      ...
                                      if args.mm_emb else (3 + args.k_ref * 3) * args.N_point_ray_enc,
    ...
    model_refine = MinMaxRayS1Conv_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                  input_ch=args.k_ref2 * 3 + input_ch * args.N_point_ray_enc if args.mm_emb else
                                  (3 + args.k_ref2 * 3) * args.N_samples,
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, H, Wfull, W, K, k_ref,
    ...
    with torch.no_grad():
        ...
        K_ = K.unsqueeze(0).repeat(N_point_ray_enc * k_ref, 1, 1)
        inv_K = torch.inverse(K_)
    ...
    warps = inverse_warp.inverse_warp_rod1_rt2_coords(ref_rgb, depths, ro1, rd1, ref_pose, K_, inv_K,
    ...
    mm_input = torch.cat((warps_flat.view(1, k_ref*N_point_ray_enc * 3, H, W),
                          torch.permute(pts.view(1, H, W, N_point_ray_enc * 3), (0, 3, 1, 2)),
                          ), 1)
    ...
    mean_epi_depth = torch.mean(epi_z_vals, dim=-1, keepdim=True)
    epi_pts = or_rays_o[..., None, :] + or_rays_d[..., None, :] * epi_z_vals[..., :, None]
    rgb0 = min_max_rays[:, N_samples::]

    epi_pts = epi_pts.reshape(H*W, N_samples * 3)
    # if embed_fn is not None:
    #     epi_pts = embed_fn(epi_pts)

    with torch.no_grad():
        k_ref = 16 #len(ref_nos)
        rel_cam_dist = torch.sum((target_pose[None, :, 3] - ref_poses[:, :, 3]) ** 2, 1) ** (1 / 2)
        _, rel_cam_idx = torch.sort(rel_cam_dist.detach(), dim=0)
        ref_nos = rel_cam_idx.int().tolist() # use all epipolar
        ref_nos = ref_nos[0:k_ref]
        ref_rgb = ref_rgbs[ref_nos].view(k_ref, Hfull, Wfull, 3)
        ref_rgb = torch.permute(ref_rgb, (0, 3, 1, 2))
        ...
        K_ = K.unsqueeze(0).repeat(N_samples * k_ref, 1, 1)
        inv_K = torch.inverse(K_)
    ...
    rgb1 = torch.permute(min_max_rays.view(3, H*W), (1, 0))

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb1, 'depth_map':mean_epi_depth, 'mean_warp':rgb0}
