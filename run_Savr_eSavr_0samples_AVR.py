...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, default='configs/fern/fern_0sample_AVR.txt',
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if savedir is not None:
            ...
            # for nnn in range(mm_rgbs[-1].shape[2]):
            #     rgb8 = to8b(mm_rgbs[-1][:, :, nnn])
            #     filename = os.path.join(savedir, 'mm_rgb{}_{:03d}.png'.format(nnn, i))
            #     imageio.imwrite(filename, rgb8)

            # dmap = to8b(avr0_depths[-1] / np.percentile(avr0_depths[-1], 99))
            # filename = os.path.join(savedir, '{:03d}_AVR0_depth.png'.format(i))
            # imageio.imwrite(filename, dmap)

            # dmap = to8b(avr1_depths[-1] / np.percentile(avr1_depths[-1], 99))
            # filename = os.path.join(savedir, '{:03d}_AVR1_depth.png'.format(i))
            # imageio.imwrite(filename, dmap)
...
def create_nerf(args):
    ...
    model_refine = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                input_ch=6 * args.N_samples + 3 * args.num_neighbor * args.N_samples +
                                         3 * args.N_samples + 3 + (3 * 4) * args.num_neighbor,
                                output_ch=4*args.num_neighbor * args.N_samples + args.num_neighbor, skips=args.mmnetskips)
...
def raw2outputs(densities, rgbs, z_vals, rays_d, white_bkgd=False):
    """Transforms model's predictions to semantically meaningful values.
    Args:
        densities: [N_rays, N_samples, N_n]
        rgbs: [N_rays, N_samples, N_n, 3]
    ...
    alpha = raw2alpha(densities, dists)  # [N_rays, N_samples, N_n]
    weights = alpha * torch.cumprod(
        torch.cat([torch.ones((alpha.shape[0], 1, alpha.shape[-1])), 1. - alpha + 1e-10], 1), 1)[:, :-1, :]
    rgb_map = torch.sum(weights[..., None] * rgbs, 1)  # [N_rays, N_n, 3]
    # rgb_map = rgb_map[:, 0, :] # [N_rays, 3]

    acc_map = torch.sum(weights, 1) # [N_rays, N_n]
    acc_map = torch.mean(acc_map, 1)

    depth_map = torch.sum(weights * z_vals[..., None], 1)
    depth_map = torch.mean(depth_map, 1)
    disp_map = 1. / torch.max(1e-10 * torch.ones_like(depth_map), depth_map / torch.sum(torch.sum(weights, 1), 1))
...
def render_rays(ray_batch, or_ray_batch,
    ...
    F_theta0_out = min_max_ray_net(plucker_pts)
    depth_values = torch.sigmoid(F_theta0_out[:, :N_samples]) * (far - near) + near
    ...
    ref_pose_in = ref_poses[None].view(1, -1, 3*4).expand(N_rays, -1, -1)
    ...
    epi_var = torch.abs(torch.var(epi_features.view(N_rays, N_samples, num_neighbor, 3), dim=2))

    weights = F.softmax(weights_f0, dim=1)
    rgb0 = torch.sum(weights.view(-1, N_samples, 1, 1) *
                       epi_features.view(N_rays, N_samples, num_neighbor, 3), dim=1)
    ...
    net_input = torch.cat((plucker_embed.view(N_rays, -1), epi_features.view(N_rays, -1), epi_var.view(N_rays, -1),
                           viewdirs.view(N_rays, -1), ref_pose_in.view(N_rays, -1)), -1)

    F_theta1_output = refine_net(net_input)
    ...
    # epi_features = epi_features.view(N_rays, N_samples, num_neighbor, 3) + \
    #                res_epi.view(N_rays, N_samples, num_neighbor, 3)

    # Approximated volumetric rendering (AVR)
    weights = F.softmax(weights0.view(N_rays, N_samples, num_neighbor), dim=1)
    refine_rgbs = torch.sum(weights.view(N_rays, N_samples, num_neighbor, 1) *
                       epi_features.view(N_rays, N_samples, num_neighbor, 3), dim=1)
    mean_depth = torch.sum(depth_values[:, :, None] * weights.view(N_rays, N_samples, num_neighbor), dim=-1)
...
def train():
    ...
    if args.dataset_type == 'llff':
        images, poses, bds, render_poses, i_test, i_ref = load_llff_data_infer(args.datadir, args.factor,
                                                                  ...
                                                                  spherify=args.spherify, num_neighbor=None)
        hwf = poses[0, :3, -1]
        poses = poses[:, :3, :4]
    ...
    for i in trange(start, N_iters):
        ...
        #     with torch.no_grad():
        #         r_out = render_path(render_poses, hwf, K, args.chunk, render_kwargs_test, gt_imgs=None, savedir=None)
        #         rgbs0, rgbs1, depths0, depths = r_out[0], r_out[1], r_out[2], r_out[3]
