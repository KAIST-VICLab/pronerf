...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True, 
    ...
    if c2w is not None:
        ...
        the_p_uv = torch.stack((p_u.t(), p_v.t()), -1)
    else:
    ...
    kwargs['the_p_uv'] = the_p_uv.reshape(-1,2)
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        max_offset.append(torch.abs(extras['max_offset'] ).cpu().numpy())
        mean_offset.append(torch.abs(extras['mean_offset']).cpu().numpy())
        ...
        if savedir is not None:
            ...
            # rgb8 = to8b(max_offset[-1]/np.max(max_offset[-1]))
            # filename = os.path.join(savedir, 'max_offset_{:03d}.png'.format(i))
            # heatmap = cv2.applyColorMap(rgb8, cv2.COLORMAP_MAGMA)
            # # imageio.imwrite(filename, rgb8)
            # cv2.imwrite(filename, heatmap)

            rgb8 = to8b(mean_offset[-1]/np.max(mean_offset[-1]))
            filename = os.path.join(savedir, 'mean_offset_{:03d}.png'.format(i))
            # imageio.imwrite(filename, rgb8)
            heatmap = cv2.applyColorMap(rgb8, cv2.COLORMAP_MAGMA)
            # imageio.imwrite(filename, rgb8)
            cv2.imwrite(filename, heatmap)
            ...
            # rgb8 = to8b(mean_offset[-1]/np.max(mean_offset[-1]))
            filename = os.path.join(savedir, 'mean_offset_{:03d}.npy'.format(i))
            # imageio.imwrite(filename, rgb8)
            # heatmap = cv2.applyColorMap(rgb8, cv2.COLORMAP_MAGMA)
            # imageio.imwrite(filename, rgb8)
            np.save(filename, mean_offset[-1])
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False, mm_density_add=None, mm_density_mul=None, iter=1e6, kwargs=None):
    ...
    # if kwargs['img_id'] == 1:
    #     if torch.any(kwargs['the_p_uv'][:,0] == 881) and torch.any(kwargs['the_p_uv'][:,1] == 364):
    #         breakpoint()
    # index = (kwargs['the_p_uv'][:,0] == 881) & (kwargs['the_p_uv'][:,1] == 364)
    ...
    # if False:
        ...
        # if False:
            ...
            # alpha[:, :-1] = alpha[:, :-1].clone() * torch.relu(mm_density_mul[:, :-1].clone())
    else:
...
def render_rays(ray_batch, or_ray_batch,
    ...
    points_offset = torch.norm((1e-2) * points_offset, dim =-1)
    ...
    rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std, white_bkgd,
                                                                 ...
                                                                 mm_density_mul=mm_density_mul, iter=iter, kwargs =kwargs)

    # uniform
    # uniform_z_vals =  torch.linspace(0.0, 0.9983, 256)* (far - near) + near 
    # query_points_nerf = rays_o[..., None, :] + rays_d[..., None,
    #                                             :] * uniform_z_vals[..., :, None]
    # raw = network_query_fn(query_points_nerf, viewdirs, network_fine)
    # rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, uniform_z_vals, rays_d, raw_noise_std, white_bkgd,
    #                                                              pytest=pytest, mm_density_add=mm_density_add,
    #                                                              mm_density_mul=mm_density_mul, iter=iter, kwargs =kwargs)

    particle_index = torch.argmax(weights, dim =1)[:,None]
    max_offset = torch.gather(points_offset, index=particle_index, dim = 1)
    mean_offset = (points_offset*weights).sum(-1, keepdim=True)
    ret = {'rgb_map0': refine_rgb, 'rgb_map1': rgb_map, 'depth_map': depth_map, 'mm_rgb': mm_rgb, 'z_vals': epi_z_vals.mean(dim=-1), 'z_vals0': depth_values.mean(dim=-1),
           'max_offset': max_offset, 'mean_offset': mean_offset}
    return ret
