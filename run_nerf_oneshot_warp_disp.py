...
def create_nerf(args):
    ...
    model_refine = MinMaxRayS1Conv_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                  ...
                                  args.k_ref * 3 + 3 * args.N_point_ray_enc,
                                  output_ch=args.k_ref * args.N_point_ray_enc, skips=args.mmnetskips)
...
def render_rays(ray_batch, coords, target_pose, ref_poses, ref_rgbs, H, W, K, k_ref,
    ...
    with torch.no_grad():
        ...
        if k_ref > 0:
            ...
            warps_flat = warps.clone() #torch.permute(warps, (2, 3, 0, 1))
            # warps_flat = warps_flat.view(H * W, k_ref * N_point_ray_enc * 3)
            warps_flat = warps_flat.view(1, k_ref, N_point_ray_enc, 3, H, W)
    ...
    if k_ref > 0:
        # ref_o = ref_poses[ref_nos].view(k_ref, 3, 4)[:, :, 3].view(1, k_ref * 3).repeat(H * W, 1)
        # mm_input = torch.cat((warps_flat, ref_o, pts), 1)
        ref_o = ref_poses[ref_nos].view(k_ref, 3, 4)[:, :, 3].view(1, k_ref * 3, 1, 1).repeat(1, 1, H, W)
        mm_input = torch.cat((ref_o, torch.permute(pts.view(1, H, W, -1), (0, 3, 1, 2))), 1)
        weights = refine_net(mm_input)
        pvolume = torch.softmax(weights.view(1, k_ref, N_point_ray_enc, 1, H, W), 2)
        warps_flat = warps_flat * pvolume
        depth_map = torch.sum(pvolume * z_vals.view(1, 1, N_point_ray_enc, 1, 1, 1), 2).view(k_ref, H, W)
        warps = torch.sum(warps_flat.view(k_ref, N_point_ray_enc, 3, H, W), 1)
        mm_input = torch.cat((warps_flat.view(1, k_ref * N_point_ray_enc * 3, H, W),
                              ref_o, torch.permute(pts.view(1, H, W, -1), (0, 3, 1, 2))), 1)
    else:
        # mm_input = pts
        mm_input = torch.permute(pts.view(1, H, W, -1), (0, 3, 1, 2))

    # mm_input_c = torch.permute(mm_input.view(1, H, W, -1), (0, 3, 1, 2))

    rgb0 = min_max_ray_net(mm_input)

    rgb0 = torch.permute(rgb0.view(3, H*W), (1, 0))

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb0, 'depth_map':depth_map.view(-1, 1), 'mean_warp':rgb0}
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        warp_loss = 0
        for num_ref in range(args.k_ref):
            this_warped_view = extras[f'warp_{num_ref}']
            valid_mask = (torch.sum(this_warped_view.detach(), dim=1, keepdim=True) != 0).type_as(this_warped_view)
            warp_loss = warp_loss + torch.mean(valid_mask * (this_warped_view - target_s) ** 2)
