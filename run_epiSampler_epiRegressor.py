...
def batchify_rays(rays, or_rays, coords, target_pose, ref_poses, ref_rgbs,
    ...
    for i in range(0, rays.shape[0], chunk):
        ret = render_rays(rays[i:i+chunk], or_rays[i:i+chunk], coords, target_pose, ref_poses, ref_rgbs,
                          H, W, K, ref_K, k_ref, rp_k_ref, **kwargs)
        for k in ret:
            if k not in all_ret:
                all_ret[k] = []
...
def render(H, W, K, ref_K, k_ref, rp_k_ref, coords=None, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, or_rays, coords, target_pose, ref_poses, ref_rgbs,
                          H, W, K, ref_K, k_ref, rp_k_ref, **kwargs)
    for k in all_ret:
...
def render_path(render_poses, hwf, K, k_ref, rp_k_ref, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        # print(i, time.time() - t)
        t = time.time()
        rgb0, rgb1, depth, nerf_depth, extras = render(H, W, K, torch.Tensor(K.copy()).to(device), k_ref, rp_k_ref,
        ...
        if 'rgb_map_n0' in extras.keys():
            rgbs_n0.append(extras['rgb_map_n0'].cpu().numpy())
...
def create_nerf(args):
    ...
    model_mmray = MinMaxRay_NetEpiNPE0(D=args.mmnetdepth, W=args.mmnetwidth,
                                       input_points=args.N_point_ray_enc,
                                       input_ch=input_ch if args.mm_emb else 3,
                                       input_epi=3 * args.rp_k_ref,
    ...
    # model_refine = MinMaxRay_NetEpiNPE0(D=args.mmnetdepth, W=args.mmnetwidth,
    #                                     input_points=args.N_samples,
    #                                     input_ch=input_ch if args.mm_emb else 3,
    #                                     input_epi=3 * args.k_ref,
    #                                     output_ch=3, skips=args.mmnetskips)
    model_refine = MinMaxRayAttn_Net(D=4, W=64,
                                        input_ch=3 + 32,
                                        pos_enc=embed_fn,
                                        view_enc=embeddirs_fn,
                                        posenc_dim=input_ch,
                                        viewenc_dim=input_ch_views,
                                        output_ch=3)
    
    grad_vars.append({'params': model_refine.parameters(), 'weight_decay': args.weight_decay, 'lr': args.lrate})

    model_feature = ResUNet(coarse_out_ch=32, fine_out_ch=32, single_net=True)
    grad_vars.append({'params': model_feature.parameters(), 'weight_decay': args.weight_decay, 'lr': args.lrate})
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        model_feature.load_state_dict(ckpt['feature_net_state_dict'])
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, rgb0=None,
    ...
    if rgb0 is None:
        ...
        rgb = rgb0 + raw[..., :3]  # [N_rays, N_samples, 3]
        # rgb = torch.sigmoid(rgb0) + raw[..., :3]  # [N_rays, N_samples, 3]
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    if rp_k_ref > 0:
        with torch.no_grad():
            ...
            warps, _ = inverse_warp.inverse_warp_rod1_rt2_coords(ref_rgb, depths, ro1, rd1, ref_pose, ref_K_, inv_K,
                                                              padding_mode='zeros')
            warps_flat = warps.clone().view(1, rp_k_ref, N_point_ray_enc, 3, 1, N_rays)


            warps_flat = torch.permute(warps_flat.view(rp_k_ref, N_point_ray_enc, 3, N_rays),(3, 1, 0, 2)).reshape(N_rays, N_point_ray_enc, 3 * rp_k_ref)

        rgb00 = None
        min_max_rays = min_max_ray_net(pts, warps_flat)
    else:
        rgb00 = None
    ...
    epi_z_vals[:, 0] = raw_z_vals[:, 0] * (far - near) + near
    for n_level in range(1, N_samples):
        epi_z_vals[:, n_level] = raw_z_vals[:, n_level] * (far + min_dist - epi_z_vals[:, n_level - 1]) \
                                 + epi_z_vals[:, n_level - 1]

    mean_epi_depth = torch.mean(epi_z_vals, 1, True)
    z_densities_a = min_max_rays[:, N_samples:N_samples * 2]
    z_densities_b = F.relu(min_max_rays[:, N_samples * 2:N_samples * 3])
    rgb0 = min_max_rays[:, N_samples * 3::]
    ...
    if k_ref > 0:
        with torch.no_grad():
            ...
            ref_nos = rel_cam_idx.int().tolist()
            if randomize:
                k_ref = k_ref + np.random.randint(low=-2, high=2)
            ...
            ref_feat = feature_net(ref_rgb)[0]

            ref_rgb = torch.repeat_interleave(ref_rgb, repeats=N_samples, dim=0)
            ref_feat = torch.repeat_interleave(ref_feat, repeats=N_samples, dim=0)
        ...
        warps, warps_feat, valid_mask = inverse_warp.inverse_warp_rod1_rt2_coords_feat(ref_rgb, ref_feat, depths, ro1, rd1, ref_pose, ref_K, inv_K,
                                                          padding_mode='zeros')
        warps_flat = warps.clone().view(1, k_ref, N_samples, 3, 1, N_rays)
        warps_feat_flat = warps_feat.clone().view(1, k_ref, N_samples, -1, 1, N_rays)

        valid_mask_flat = valid_mask.clone().view(1, k_ref, N_samples, 1, 1, N_rays)

        warps_flat = torch.permute(warps_flat.view(k_ref, N_samples, 3, N_rays), (3, 1, 0, 2))
        warps_feat_flat = torch.permute(warps_feat_flat.view(k_ref, N_samples, -1, N_rays), (3, 1, 0, 2))
        attn_mask = torch.permute(valid_mask_flat.view(k_ref, N_samples, 1, N_rays), (3, 1, 0, 2))

        # preprare ray diff, compyte 3d sampling points
        epi_pts_3d = or_rays_o[..., None, :] + or_rays_d[..., None, :] * z_vals_world[..., :, None]
        train_poses = ref_poses[ref_nos][None].repeat(N_rays,1,1,1)

        ray_diff = compute_angle(epi_pts_3d, target_pose[None].repeat(N_rays,1,1), train_poses).clone().detach()

        rgb10 = None
        epi_pts = rays_o[..., None, :] + rays_d[..., None, :] * epi_z_vals[..., :, None]
        min_max_rays = refine_net(torch.cat([warps_feat_flat,warps_flat],-1), ray_diff, attn_mask, epi_pts, rays_d)
    else:
        rgb10 = None
        min_max_rays = refine_net(pts)
    rgb1 = min_max_rays[:, -3::]

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb1, 'depth_map':mean_epi_depth, 'nerf_depth':rgb1[:, 0, None],
           'rgb_map_n0': torch.abs(rgb0 - rgb1)}

    if rgb00 is not None:
        rgb00 = rgb00[:, -3::]
        ret['rgb_map00'] = rgb00
    if rgb10 is not None:
        rgb10 = rgb10[:, -3::]
        ret['rgb_map10'] = rgb10
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            imageio.mimwrite(moviebase + 'rgbsn0.mp4', to8b(rgbsn0), fps=30, quality=8)
    ...
    for i in trange(start, N_iters):
        with torch.no_grad():
            ...
            # if random.random() > 0.5:
            #     ref_rgbs = [blur_ref_rgbs[n_ref, :, :, :] for n_ref in range(rays_rgb.shape[0]) if n_ref != img_i]
            # else:
            ...
            select_inds = random.sample(range(H*W), N_rand)
        ...
        rgb0, rgb1, depth, nerf_depth, extras = render(H, W, K, ref_K, args.k_ref, args.rp_k_ref,
        ...
        if args.a_mmdisp > 0 and i >= a_sch:
            depth_loss = torch.mean(torch.abs(depth - nerf_depth))

        loss = img_loss + args.a_mmrgb * img2mse(rgb0, target_s) + args.a_mmdisp * depth_loss

        if 'rgb_map00' in extras:
            loss += img2mse(extras['rgb_map00'], target_s)
        if 'rgb_map10' in extras:
            loss += img2mse(extras['rgb_map10'], target_s)
        ...
        if i % args.i_weights == 0:
            ...
            if args.N_importance > 0:
                torch.save({
                    ...
                    'feature_net_state_dict': render_kwargs_train['feature_net'].state_dict(),
