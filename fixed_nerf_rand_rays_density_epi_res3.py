...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        sigma_loss = 0
        if i < 30000:
            sigma_loss = (1e-5)*(-(extras['sigma']).mean()) # sigma loss for density
        elif i > 130000:
            sigma_loss = 0
        else:
            sigma_loss = (1e-6)*(-(extras['sigma']).mean()) # sigma loss for density

        loss = img_loss + rgb0_loss + depth_loss + sigma_loss
