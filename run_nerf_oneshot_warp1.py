...
def create_nerf(args):
    ...
    model_mmray = MinMaxRayS1Conv_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                      input_ch=args.k_ref * input_ch + (input_ch + args.k_ref * 3) * args.N_point_ray_enc
                                      if args.mm_emb else args.k_ref * input_ch + (3 + args.k_ref * 3) * args.N_point_ray_enc,
...
def render_rays(ray_batch, coords, target_pose, ref_poses, ref_rgbs, H, W, K, k_ref,
    ...
    if k_ref > 0:
        ...
        ref_o = embed_fn(ref_poses[ref_nos].view(k_ref, 3, 4)[:, :, 3]).view(1, -1, 1, 1).repeat(1, 1, H, W)
    ...
    # if k_ref > 0:
    #     mm_input = torch.cat((rgb0, ref_o, torch.permute(pts.view(1, H, W, -1), (0, 3, 1, 2))), 1)
    # else:
    #     mm_input = torch.cat((rgb0, torch.permute(pts.view(1, H, W, -1), (0, 3, 1, 2))), 1)
    # rgb1 = refine_net(mm_input) + rgb0

    rgb0 = torch.permute(rgb0.view(3, H*W), (1, 0))
    # rgb1 = torch.permute(rgb1.view(3, H*W), (1, 0))

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb0, 'depth_map':rgb0, 'mean_warp':rgb0}
