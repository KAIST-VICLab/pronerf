...
def render_path(render_poses, hwf, K, k_ref, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        # print(i, time.time() - t)
        t = time.time()
        rgb, depth, extras = render(H, W, K, torch.Tensor(K.copy()).to(device), k_ref,
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K, k_ref,
    ...
    pts, epi_z_vals = compute_query_points_from_rays(or_rays_o, or_rays_d, or_near, or_far, N_samples, randomize)
    ...
    with torch.no_grad():
        ...
        depths = epi_z_vals.view(1, 1, 1, N_samples).repeat(k_ref, 1, N_rays, 1) # k_ref, H, W, N_point_ray_enc
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        rgb1, depth, extras = render(H, W, K, ref_K, args.k_ref, target_pose=pose, ref_poses=ref_poses,
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            with torch.no_grad():
                ...
                rgbs, depths, acc_maps = r_out[0], r_out[1], r_out[2]
