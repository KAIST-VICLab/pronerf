...
def render_rays(ray_batch, or_ray_batch,
    ...
    min_max_rays = min_max_ray_net(pts, mm_epi_features)
    ...
    with torch.no_grad():
        ...
        epi_features = (valid_warps_flat.view(num_pts, num_neighbor, 3, warp_H*warp_W).permute(3,0,1,2)).view(N_rays, num_pts, -1) # N rays, 3*num_pts
    ...
    refine_output = refine_net(epi_pts, epi_features)
    ...
    ret = {'rgb_map0': refine_rgb, 'rgb_map1': rgb_map,'depth_map': depth_map, 'sigma1': raw[..., 3], 'mm_rgb': mm_rgb}
    return ret
