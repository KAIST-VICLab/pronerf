...
def inverse_warp_rt1_rt2(img, depth, c2w1, c2w2, intrinsics, intrinsics_inv, padding_mode='zeros'):
    """
    Inverse warp a source image to the target image plane.
...
def inverse_warp_rod1_rt2(img, depth, ro1, rd1, c2w2, intrinsics, intrinsics_inv, scale=1., padding_mode='zeros'):
    """
    Inverse warp a source image to the target image plane.
...
def inverse_warp_rod1_rt2_coords(img, depth, ro1, rd1, c2w2, intrinsics, intrinsics_inv, scale=1., padding_mode='zeros'):
    ...
    if scale != 1:
        ...
        projected_img = torch.nn.functional.grid_sample(img, src_pixel_coords, mode='bilinear', padding_mode=padding_mode,
                                                    align_corners=True)
