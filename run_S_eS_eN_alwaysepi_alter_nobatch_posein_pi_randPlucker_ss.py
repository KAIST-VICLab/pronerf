...
# lite nets:
# Ficus
# Mean Test PSNR RGB1 28.685102462768555
# Mean Test PSNR RGB0 23.908287048339844
# [TRAIN] Iter: 360000 Loss: 0.05738971009850502  PSNR_n: 23.364418029785156  PSNR_sn: 24.620389938354492
# Mic
# Mean Test PSNR RGB1 33.15877914428711
# Mean Test PSNR RGB0 28.627229690551758
#  90%|████████▉ | 440001/490001 [8:15:17<141:38:34, 10.20s/it]Saved test set

def config_parser():
    import configargparse
    parser = configargparse.ArgumentParser()
    parser.add_argument('--config', is_config_file=True, default='configs/mic/mic_epinerf.txt',
...
def render(H, W, K, ref_K, k_ref, N_n, coords=None, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
           target_pose=None, ref_poses=None, ref_rgbs=None, p_uv=None,
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, or_rays, target_pose, ref_poses, ref_rgbs, p_uv,
...
def render_path(render_poses, hwf, K, k_ref, N_n, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        rgb0, rgb1, depth, nerf_depth, extras = render(H, W, K, torch.Tensor(K.copy()).to(device), k_ref, N_n,
                                                       target_pose=c2w[:3, :4], ref_poses=ref_poses, p_uv=p_uv,
...
def create_nerf(args):
    ...
    model_mmray = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                input_ch=2 + 9 * args.N_point_ray_enc
    ...
    model_refine = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                 input_ch=2 + 9 * args.N_samples + 3 * args.N_n * args.N_samples + 3 * args.N_n
    ...
    render_kwargs_train = {
        ...
        'embed_fn': embed_fn_sampler if args.mm_emb else None,
        'randomize': True
    }
...
def render_rays(ray_batch, or_ray_batch, p_uv, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
                k_ref, N_n,
                network_fn,
                network_query_fn,
                N_samples,
    ...
    pts, z_vals = compute_query_points_from_rays(rays_o, rays_d, near, far, N_point_ray_enc, randomize)

    p_uv = 0 * p_uv
    m = pts
    if embed_fn is not None:
        plucker_pts = embed_fn(pts, rays_d[:, None, :].repeat(1, N_point_ray_enc, 1))
        plucker_pts = torch.cat([plucker_pts, pts], dim=-1)
        m = plucker_pts.view(-1, (N_point_ray_enc) * 9)
    m = m.view(N_rays, -1)
    m = torch.cat((m, p_uv), -1)  # + puv
    ...
    if False:  # train_sampler and randomize:
        # Explore bewteen predicted samples
        noise_shape = [N_rays, N_samples]
        max_noise = 0.99
        noise_ = (1 / 5) * torch.normal(0.0, 1.0, size=noise_shape).type_as(depth_values)
        noise_ = torch.abs(noise_)
        noise_[noise_ > max_noise] = max_noise
        if random.random() > 0.5:
            epi_z_vals_diff = torch.abs(
                depth_values - torch.cat((depth_values[:, 1:], far * torch.ones(N_rays, 1).type_as(depth_values)), 1))
            noise_ = noise_ * epi_z_vals_diff  # N_Rays, N_samples
        else:
            epi_z_vals_diff = torch.abs(
                depth_values - torch.cat((near * torch.ones(N_rays, 1), depth_values[:, 0:-1].type_as(depth_values)),
                                         1))
            noise_ = -noise_ * epi_z_vals_diff
        depth_values = depth_values + noise_

    N_point_ray_enc = N_samples
    with torch.no_grad():
        # select epi neighbours
        rel_cam_dist = torch.sum((target_pose[None, :, 3] - ref_poses[:, :, 3]) ** 2, 1) ** (1 / 2)
        _, rel_cam_idx = torch.sort(rel_cam_dist.detach(), dim=0)
        rel_cam_idx = rel_cam_idx[0:N_n * 5]
        if randomize:
    ...
    m = epi_pts
    if embed_fn is not None:
        plucker_pts = embed_fn(epi_pts, rays_d[:, None, :].repeat(1, N_samples, 1))
        plucker_pts = torch.cat([plucker_pts, epi_pts], dim=-1)
        m = plucker_pts.view(-1, (N_point_ray_enc) * 9)
    m = m.view(N_rays, -1)
    ...
    if randomize and not train_sampler:
        ...
        if n_mult > 1:
            ...
            if random.random() > 0.5:
                epi_z_vals_diff = torch.abs(
                    refine_depth_values -
                    torch.cat((refine_depth_values[:, 1:], far * torch.ones(N_rays, 1).type_as(refine_depth_values)),
                              1))
                noise_ = mults[:, None, :] * epi_z_vals_diff[:, :, None]  # N_Rays, N_samples, n_mult
            else:
                epi_z_vals_diff = torch.abs(
                    refine_depth_values -
                    torch.cat((near * torch.ones(N_rays, 1), refine_depth_values[:, 0:-1].type_as(refine_depth_values)),
                              1))
    ...
    with torch.no_grad():
        ...
        rel_cam_dist = torch.sum((target_pose[None, :, 3] - ref_poses[:, :, 3]) ** 2, 1) ** (1 / 2)
        _, rel_cam_idx = torch.sort(rel_cam_dist.detach(), dim=0)
        rel_cam_idx = rel_cam_idx[0:N_n0 * 5]
        if randomize:
            # Random but keep order
            ref_nos = rel_cam_idx.int().tolist()
            order_idx = sorted(random.sample(range(len(ref_nos)), N_n0))
            ref_nos = [ref_nos[i] for i in order_idx]
        else:
            # Nearest
            ref_nos = rel_cam_idx[0:N_n0].int().tolist()

        C_color = ref_rgbs.shape[-1]
        ref_rgb = ref_rgbs[ref_nos].view(N_n0, Hfull, Wfull, C_color)
        ref_rgb = torch.permute(ref_rgb, (0, 3, 1, 2))
        ref_rgb = torch.repeat_interleave(ref_rgb, repeats=N_point_ray_enc, dim=0)
        ref_pose = ref_poses[ref_nos]
        ref_pose = torch.repeat_interleave(ref_pose, repeats=N_point_ray_enc, dim=0)
        ...
        ro1, rd1 = torch.transpose(or_rays_o, 0, 1).unsqueeze(0), torch.transpose(or_rays_d, 0, 1).unsqueeze(
            0)  # 1, 3, H*W
        ro1, rd1 = ro1.repeat(N_point_ray_enc * N_n0, 1, 1), rd1.repeat(N_point_ray_enc * N_n0, 1, 1)
        ref_K_ = ref_K.unsqueeze(0).repeat(N_point_ray_enc * N_n0, 1, 1)
        inv_K = torch.inverse(ref_K_)

    depths = refine_depth_values.view(1, 1, N_rays, N_point_ray_enc).repeat(N_n0, 1, 1,
                                                                            1)  # k_ref, H, W, N_point_ray_enc
    depths = torch.permute(depths, (0, 3, 1, 2)).reshape(-1, 1, N_rays)  # k_ref * N_point_ray_enc, H, W
    ...
    input_ref_posesn = ref_poses[ref_nos, :, 3].view(1, N_n * 3).expand(N_rays, N_n * 3)
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            render_path(torch.Tensor(poses[i_test]).to(device), hwf, K, args.k_ref, args.N_n, args.chunk,
                        ref_poses=poses_train, p_uv=the_p_uv.view(-1, 2),
                        ref_rgbs=the_ref_rgb, render_kwargs=render_kwargs_test,
    ...
    # print(f'All coords {all_valid_coords.shape}')
    ...
    for i in trange(start, N_iters):
        ...
        if i >= 30000 and i%101 == 0: # this is self distillation
            render_kwargs_train['train_nerf'] = False
            render_kwargs_train['train_sampler'] = True
            ss_img_i = random.choice(range(render_poses.shape[0]))
            ss_pose = torch.Tensor(render_poses[ss_img_i]).to(device)

            rgb0, rgb1, depth, nerf_depth, extras = render(H, W, K, ref_K, args.k_ref, args.N_n,
                                                        target_pose=ss_pose[:3, :4], ref_poses=poses_train, p_uv=the_p_uv.view(-1, 2),
                                                        ref_rgbs=the_ref_rgb, chunk=args.chunk, c2w=ss_pose[:3, :4],
                                                        **render_kwargs_train)
            s_optimizer.zero_grad()
            loss = img2mse(rgb0, rgb1.detach()) + img2mse(extras['mm_rgb'], rgb1.detach())
            loss.backward()
            s_optimizer.step()
            ...
            if i % 2 != 0:
                ...
                s_optimizer.zero_grad()
                depth_loss = 0
                if args.a_mmdisp > 0 and i >= a_sch:
                    depth_loss = torch.mean((depth.squeeze() - nerf_depth.squeeze().detach()) ** 2)
                loss = img2mse(rgb1, target_s) + args.a_mmdisp * depth_loss + \
                    args.a_mmrgb * (img2mse(rgb0, target_s) + img2mse(extras['mm_rgb'], target_s))  # \
                # + img2mse(extras['rgb_map_r0'], target_s)
        ...
        if i % args.i_weights == 0:
            ...
            if args.N_importance > 0:
                torch.save({
                    ...
                    # 'embed_fn': render_kwargs_train['embed_fn'].state_dict() if args.mm_emb else None,
