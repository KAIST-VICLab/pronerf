...
class MMEngine(object):
    ...
    def __init__(self, load_model, batch=800*800//CHUNK_RATIO, in_ch=2 + 3 * N_POINT_RAY_ENC):
        t1 = time.time()
        self.in_ch = in_ch
...
class RefineEngine(object):
    ...
    def __init__(self, load_model, batch=800*800//CHUNK_RATIO, in_ch=2 + 3 * N_SAMPLES + 3 * NUM_NEIGHBOR * N_SAMPLES + 3 * NUM_NEIGHBOR):
        t1 = time.time()
...
class NeRFEngine(object):
    ...
    def __init__(self, load_model, batch=800*800*N_SAMPLES//CHUNK_RATIO, in_ch=116):
        t1 = time.time()
        self.batch_size = batch
        self.in_ch = in_ch
        ...
        # NOTE INPUT GPU MEM
        self.input_gpu = gpuarray.empty(batch*self.in_ch, dtype=np.float32)
        self.input_host_mem = cuda.pagelocked_empty((batch,self.in_ch), dtype=np.float32)
        self.input_gpu_host_mem = None
