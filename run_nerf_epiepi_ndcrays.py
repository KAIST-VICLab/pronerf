...
def render(H, W, K, ref_K, k_ref, rp_k_ref, coords=None, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
    ...
    all_ret = batchify_rays(rays, or_rays, target_pose, ref_poses, ref_rgbs,
                          H, W, K, ref_K, k_ref, rp_k_ref, chunk, **kwargs)
    for k in all_ret:
...
def render_rays(ray_batch, or_ray_batch, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    epi_z_vals = torch.sigmoid(min_max_rays[:, 0:N_samples]) * (far - near) + near
    ...
    epi_z_vals, sort_indices = torch.sort(epi_z_vals, dim=-1)
    z_densities_a = z_densities_a.gather(dim=1, index=sort_indices.detach())
    z_densities_b = z_densities_b.gather(dim=1, index=sort_indices.detach())
    ...
    # Shift and scale in densities
    raw = torch.cat((raw[:, :, :3], (raw[:, :, 3] + z_densities_a).unsqueeze(2)), -1)
    rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std, white_bkgd,
                                                                 depth_densities=z_densities_b, pytest=pytest)

    # No residual densities
    # rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std, white_bkgd,
    #                                                              pytest=pytest)

    # rgb_map, disp_map, acc_map, weights, depth_map = sraw2outputs(raw, epi_z_vals, white_bkgd)

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb_map, 'depth_map':mean_epi_depth, 'nerf_depth':depth_map,
           'rgb_map_n0': torch.abs(rgb0 - rgb_map)}
