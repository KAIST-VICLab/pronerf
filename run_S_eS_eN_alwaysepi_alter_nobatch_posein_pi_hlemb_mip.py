...
def create_nerf(args):
    ...
    # model = NeRF(D=args.netdepth_fine, W=args.netwidth_fine,
    #                     input_ch=420, output_ch=output_ch, skips=skips,
    ...
    model_mmray = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                ...
                                if args.mm_emb else 2 + 9 * args.N_point_ray_enc,
    ...
    model_refine = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                 ...
                                 if args.mm_emb else 2 + 9 * args.N_samples + 3 * args.N_n * args.N_samples +
                                                     3 * args.N_n,
...
def render_rays(ray_batch, or_ray_batch, p_uv, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    pts, z_vals = compute_query_points_from_rays(rays_o, rays_d, near, far, N_point_ray_enc, randomize=randomize)

    # m = pts
    # if embed_fn is not None:
    #     m = embed_fn(pts.view(-1, 3))
    #     m = m.view(N_rays, N_point_ray_enc, -1)
    # m = m.view(N_rays, -1)
    # m = torch.cat((m, p_uv), -1)

    plucker_pts = kwargs['embed_rays'](pts, rays_d[:, None, :].repeat(1, N_point_ray_enc, 1))
    plucker_pts = torch.cat([plucker_pts, pts], dim =-1) 
    plucker_pts = plucker_pts.view(-1, (N_point_ray_enc) * 9)
    m = torch.cat((plucker_pts, p_uv), -1) # + puv
    ...
    plucker_pts = kwargs['embed_rays'](epi_pts, rays_d[:, None, :].repeat(1, N_samples, 1))
    plucker_pts = torch.cat([plucker_pts, epi_pts], dim =-1) 
    m = plucker_pts

    # m = epi_pts
    # if embed_fn is not None:
    #     m = embed_fn(epi_pts.view(-1, 3))
    #     m = m.view(N_rays, N_point_ray_enc, -1)
...
def train():
    ...
    rays_radii = np.stack([rays_radii[i] for i in i_train], 0)
   
    rays_rgb = rays_rgb.astype(np.float32)
    rays_radii = rays_radii.astype(np.float32)
    print('done')
    ...
    for i in trange(start, N_iters):
        with torch.no_grad():
            ...
            these_rays_radii = rays_radii[img_i]
            ...
            these_rays_radii = these_rays_radii.view(H * W, 1)[select_inds]
            ...
            render_kwargs_train['rays_radii'] = these_rays_radii
