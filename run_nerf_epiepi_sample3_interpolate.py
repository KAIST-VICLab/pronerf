...
def create_nerf(args):
    ...
    output_ch = 32#5 if args.N_importance > 0 else 4
    skips = args.netskips
    if args.k_ref > 0:
    ...
    model_refine = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                  input_ch=(output_ch + 1) * args.N_samples + args.k_ref * 3 * args.N_samples,
                                  output_ch=10 * (4 + 1) * args.N_samples, skips=args.mmnetskips).to(device)
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    rgb_map_n0, _, _, _, _ = raw2outputs(raw[:, :, -4:], epi_z_vals, rays_d, raw_noise_std, white_bkgd, pytest=pytest)

    # Get more points for vol rend
    if k_ref > 0:
        refine_input = torch.cat((epi_z_vals, warps_flat.view(N_rays, -1), raw.view(N_rays, -1)), -1)
        raw = refine_net(refine_input)
    else:
        refine_input = torch.cat((epi_z_vals, raw.view(N_rays, -1)), -1)
        raw = refine_net(refine_input)

    epi_z_vals = torch.sigmoid(raw[:, 0:10*N_samples]) * (far - near) + near
                 # (epi_z_vals[:, -1, None] - epi_z_vals[:, 0, None]) + epi_z_vals[:, 0, None]
    raw = raw[:, 10*N_samples::].view(N_rays, 10*N_samples, 4)
    epi_z_vals, sort_indices = torch.sort(epi_z_vals, dim=-1)
    raw = raw.gather(dim=1, index=sort_indices[:, :, None].repeat(1, 1, 4).detach())
    rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std, white_bkgd,
                                                                 pytest=pytest)

    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb_map, 'depth_map':mean_epi_depth, 'mean_warp':depth_map,
           'rgb_map_n0': rgb_map_n0}
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        optimizer.zero_grad()
        img_loss = torch.mean(torch.abs(rgb1 - target_s)) #img2mse(rgb1, target_s)

        # max acc mask
        n0_loss = 0
        if 'rgb_map_n0' in extras.keys():
            n0_loss = img2mse(extras['rgb_map_n0'], target_s)

        loss = img_loss + args.a_mmrgb * img2mse(rgb0, target_s) + n0_loss
