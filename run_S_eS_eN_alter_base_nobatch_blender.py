...
def batchify_rays(rays_flat, or_rays, target_pose, ref_poses, ref_rgbs,
    ...
    all_ret = {}
    if kwargs['randomize']:
        grad_mask = kwargs['grad_mask']
    for i in range(0, rays_flat.shape[0], chunk):
        if kwargs['randomize']:
            kwargs['grad_mask'] = grad_mask[i:i+chunk]
...
def create_nerf(args):
    ...
    model_mmray = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                ...
                                3 * args.N_point_ray_enc + 3 * (args.N_n//4) * args.N_point_ray_enc,
...
def compute_query_points_from_rays(
    ...
    # disp_step = torch.linspace(0., 1., N_point_ray_enc).to(ray_origins)
    # disp_values = (1/far_thresh)*disp_step + (1/near_thresh)*(1-disp_step)
...
def render_rays(ray_batch, or_ray_batch, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    with torch.no_grad():
        ...
        rel_cam_idx = rel_cam_idx[0:N_n_sampler*2]
        if randomize:
            # Random but keep order
            ref_nos = rel_cam_idx.int().tolist()
            order_idx = sorted(random.sample(range(len(ref_nos)), N_n_sampler))
            ref_nos = [ref_nos[i] for i in order_idx]
        else:
            # Nearest
            ref_nos = rel_cam_idx[0:N_n_sampler].int().tolist()

        C_color = ref_rgbs.shape[-1]
        ref_rgb = ref_rgbs[ref_nos].view(N_n_sampler, Hfull, Wfull, C_color)
        ...
        ro1, rd1 = ro1.repeat(N_point_ray_enc * N_n_sampler, 1, 1), rd1.repeat(N_point_ray_enc * N_n_sampler, 1, 1)
        ref_K_ = ref_K.unsqueeze(0).repeat(N_point_ray_enc * N_n_sampler, 1, 1)
        inv_K = torch.inverse(ref_K_)

    # This code no grad???
    depths = ray_depth_values.view(1, 1, N_rays, N_point_ray_enc).repeat(N_n_sampler, 1, 1, 1) # k_ref, H, W, N_point_ray_enc
    ...
    warps, npcoords = inverse_warp.inverse_warp_rod1_rt2_coords(ref_rgb, depths, ro1, rd1, ref_pose, ref_K_, inv_K,
                                                    padding_mode='zeros')
    warps_flat = warps.clone().view(1, N_n_sampler, N_point_ray_enc, C_color, 1, N_rays)
    ...
    warps_flat = torch.permute(warps_flat.view(N_n_sampler, N_point_ray_enc, C_color, N_rays),
                            (3, 1, 0, 2)).reshape(N_rays, N_point_ray_enc, C_color * N_n_sampler)
    ...
    # # Autosort
    # min_dist = 1e-2
    # raw_z_vals = torch.sigmoid(min_max_rays[:, :N_samples])
    # depth_values = torch.zeros(raw_z_vals.shape).type_as(raw_z_vals)
    # depth_values[:, 0] = raw_z_vals[:, 0] * (far - near) + near
    # for n_level in range(1, N_samples):
    #     depth_values[:, n_level] = raw_z_vals[:, n_level] * (far + min_dist - depth_values[:, n_level - 1]) \
    #                              + depth_values[:, n_level - 1]
    ...
    # plucker_embed = kwargs['embed_rays'](epi_pts, rays_d[:, None, :].repeat(1, N_point_ray_enc, 1)) # nump_pts + origin
    # # plucker_embed = torch.cat([plucker_embed, contract_epi_pts], dim =-1)
    # plucker_embed = plucker_embed.view(-1, N_point_ray_enc, 6).view(N_rays, -1)


    if embed_fn:
        contract_epi_pts = embed_fn(contract_epi_pts)
        # epi_pts = embed_fn(epi_pts)

    plucker_embed = epi_pts.view(N_rays, -1)
    ...
    # # Plain regression
    # refine_depth_values = torch.sigmoid(refine_output[:, :N_samples]) * (far - near) + near
    # sort_out = torch.sort(refine_depth_values, dim=-1)
    # refine_depth_values = sort_out[0]
    # points_offset = torch.gather(points_offset, dim=1, index=sort_out[1][:, :, None].repeat(1, 1, 3))

    # min_dist = 1e-6
    # raw_z_vals = torch.sigmoid(refine_output[:, :N_samples])
    # refine_depth_values = torch.zeros(raw_z_vals.shape).type_as(raw_z_vals)
    # refine_depth_values[:, 0] = raw_z_vals[:, 0] * (far - near) + near
    # for n_level in range(1, N_samples):
    #     refine_depth_values[:, n_level] = \
    #         raw_z_vals[:, n_level] * (far + min_dist - refine_depth_values[:, n_level - 1]) \
    #         + refine_depth_values[:, n_level - 1]
...
def train():
    ...
    for i in trange(start, N_iters):
        with torch.no_grad():
            ...
            alpha_mask = alpha_masks[img_i]

            # Random selection of rays
            all_coords = all_the_coords
            if i < args.precrop_iters:
                all_coords = center_coords
            ...
            grad_mask = alpha_mask.view(H*W,1)[select_inds]
        ...
        if i % 2 != 0:
            ...
            render_kwargs_train['grad_mask'] = grad_mask
            ...
            if args.a_mmdisp > 0 and i >= a_sch:
                depth_loss = torch.mean((depth - nerf_depth.detach()) ** 2)
            loss = img2mse(rgb1, target_s) + \
                   args.a_mmrgb * img2mse(rgb0, target_s) + \
                   args.a_mmdisp * depth_loss
