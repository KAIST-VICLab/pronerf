...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False, mm_density_add=None,
    ...
    if mm_density_add is not None:
        raw_density = raw[..., 3] + mm_density_add
        raw_density = torch.clamp(raw_density, min= -1e2, max=1e2)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if i % 2 != 0:
            ...
            loss = img_loss + rgb0_loss + mm_rgb_loss 
        ...
        decay_rate = 0.1
        decay_steps = args.lrate_decay * 1000
        new_lrate = args.ngp_lrate * (decay_rate ** ((global_step/2) / decay_steps))
