...
class min_max_ray_Model(torch.nn.Module):
    def __init__(self, num_encoding_functions=0, filter_size=128):
        ...
        self.layer1 = torch.nn.Conv2d(16 * 3, filter_size, kernel_size=1)
        ...
        self.layer3 = torch.nn.Conv2d(filter_size, 3+3, kernel_size=1)
...
def compute_query_points_from_rays(
  ...
  """
    ...
    # Play with input types
    # mm_input = encoding_function(torch.cat((ray_origins, ray_directions), 2))
    # mm_input = torch.cat((ray_origins, ray_directions), 2)
    mm_input = torch.linspace(near_thresh, far_thresh, 16).to(ray_origins)
    mm_input = ray_origins[..., None, :] + ray_directions[..., None, :] * mm_input[..., :, None]  # W, H, 8, 3
    mm_input = mm_input.view(ray_origins.shape[0], ray_origins.shape[1], 16 * ray_origins.shape[2])
    ...
    min_max_rays = torch.transpose(min_max_rays.squeeze(0), 0, 2)  # (width, height, 2)

    # depth_values = torch.linspace(near_thresh, far_thresh, num_samples).to(ray_origins)

    depth_values = torch.linspace(1, 0, steps=num_samples).view(1, 1, -1).type_as(ray_origins)
    ...
    # (width, height, num_samples, 3) = (width, height, 1, 3) + (width, height, 1, 3) * (width, height, num_samples, 1)
...
def run_one_iter_of_tinynerf(height, width, focal_length, tform_cam2world,
    ...
    query_points, depth_values, min_ray, max_ray, dst, mm_rgb = compute_query_points_from_rays(
        ray_origins, ray_directions, near_thresh, far_thresh, depth_samples_per_ray, encoding_function, False
    )
    ...
    return rgb_predicted, depth_map, min_ray, max_ray, dst, mm_rgb


def log_stuff(rgb_predicted, depth_map, min_ray, max_ray, dst, i, iternums, psnrs=None, train_losses=None):
    ...
    plt.subplot(163)
    plt.imshow(min_ray.squeeze().detach().cpu().numpy())
    plt.title(f"Min rays")

    plt.subplot(164)
    plt.imshow(max_ray.squeeze().detach().cpu().numpy())
    plt.title(f"Max rays")

    plt.subplot(165)
    plt.imshow(dst.squeeze().detach().cpu().numpy())
    plt.title(f"DST")
    ...
    if psnrs is not None:
        plt.savefig('ToyResults/val.png')
    elif train_losses is not None:
        plt.savefig('ToyResults/train.png')
...
optimizer = torch.optim.Adam(list(model.parameters()) +
                             list(mm_model.parameters()), lr=lr)
...
for i in range(num_iters):
    ...
    rgb_predicted, depth_map, min_ray, max_ray, dst, mm_rgb = run_one_iter_of_tinynerf(height, width, focal_length,
                                                                               target_tform_cam2world, near_thresh,
                                                                               far_thresh, depth_samples_per_ray,
                                                                               encode, get_minibatches)

    # Compute mean-squared error between the predicted and target images. Backprop!
    loss = torch.nn.functional.mse_loss(rgb_predicted, target_img) + 0.1*torch.nn.functional.mse_loss(mm_rgb, target_img)
    ...
    if i % display_every == 0:
        ...
        log_stuff(rgb_predicted, depth_map, min_ray, max_ray, dst, i, iternums, train_losses=train_losses)

        # Render the held-out view
        rgb_predicted, depth_map, min_ray, max_ray, dst, _ = run_one_iter_of_tinynerf(height, width, focal_length,
        ...
        log_stuff(rgb_predicted, depth_map, min_ray, max_ray, dst, i, iternums, psnrs=psnrs)
