...
def run_network(inputs, viewdirs, fn, embed_fn, embeddirs_fn, netchunk=1024 * 64):
    ...
    outputs_flat = fn(embedded) #batchify(fn, netchunk)(embedded)
...
def create_nerf(args):
    ...
    model_mmray = MinMaxRayS1Conv_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                      ...
                                      output_ch=2*args.N_samples + 3, skips=args.mmnetskips).to(device)
...
def render_rays(ray_batch, or_ray_batch, coords, target_pose, ref_poses, ref_rgbs, Hfull, H, Wfull, W, K, ref_K, k_ref,
                network_fn,
                network_query_fn,
                N_samples,
                retraw=False,
    ...
    min_max_rays = torch.permute(min_max_rays.view(2*N_samples + 3, H*W), (1, 0))
    ...
    if randomize is True:
        # min_z = epi_z_vals[:, 0, None].detach()
        # noise_ = torch.rand(H*W, 1).type_as(epi_z_vals)
        # low_point = noise_ * (min_z - or_near) + or_near
        #
        # max_z = epi_z_vals[:, -1, None].detach()
        # noise_ = torch.rand(H*W, 1).type_as(epi_z_vals)
        # high_point = noise_ * (or_far - max_z) + max_z
        #
        # epi_z_vals = torch.cat((low_point, epi_z_vals, high_point), -1)
    ...
    warps = inverse_warp.inverse_warp_rod1_rt2(ref_rgb, depths, ro1, rd1, ref_pose, ref_K, inv_K, padding_mode='zeros')
    ...
    warps_flat = torch.permute(warps_flat.view(k_ref, N_samples, 3, H*W), (3, 1, 0, 2)).reshape(H*W, N_samples, 3*k_ref)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        loss = img_loss + perc_loss + img2mse(rgb0, target_s) + acc_reg # + img2mse(mean_warp, target_s)
        ...
        if i % 500 == 0:
            ...
            np_depth = mean_warp.view(th, tw).detach().cpu().numpy()
            rgb8 = to8b(np_depth / np.percentile(np_depth, 99))
            filename = os.path.join(basedir, expname, 'debug', 'check_depth0.png')
            imageio.imwrite(filename, rgb8)
