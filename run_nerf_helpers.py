...
class Embedder(nn.Module):
    ...
    def embed(self, inputs):
        embed = [inputs]
        for freq in self.freq_bands:
            embed.append(torch.sin(inputs*freq))
...
class NeRFEmbedTRT(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], use_viewdirs=False, multires=None, multires_dir = None):
        """ 
        """
        super(NeRFEmbedTRT, self).__init__()
        self.D = D
        self.W = W
        ...
        embeddirs_kwargs = {
                'max_freq_log2' : multires_dir-1,
                'num_freqs' : multires_dir,
        }

        max_freq = embeddirs_kwargs['max_freq_log2']
        N_freqs = embeddirs_kwargs['num_freqs']
        self.freq_bands_view = 2.**torch.linspace(0., max_freq, steps=N_freqs)

    def forward(self, input_pts, input_views):
        freqs = (input_pts[:, None, :] * self.freq_bands[None, :, None]) # N_rays, 1, 3 * 1, N_freqs, 1 = N_rays, N_freqs, 3 -> _rays, N_freqs * 3
        sines = torch.sin(freqs)
        # sines = (16*freqs*(torch.tensor(math.pi)- freqs))/(5*(torch.tensor(math.pi))**2 -4*freqs*(torch.tensor(math.pi)- freqs))
        cosines = torch.cos(freqs)
        embed = torch.cat([sines,cosines], -1).flatten(1,2)
        input_pts = torch.cat((input_pts, embed), -1)
        

        freqs = (input_views[:, None, :] * self.freq_bands_view[None, :, None]) # N_rays, 1, 3 * 1, N_freqs, 1 = N_rays, N_freqs, 3 -> _rays, N_freqs * 3
        sines = torch.sin(freqs)
        cosines = torch.cos(freqs)
        embed = torch.cat([sines,cosines], -1).flatten(1,2)
        input_views = torch.cat((input_views, embed), -1)
...
class NeRFTRT(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[], use_viewdirs=False):
        """ 
        """
...
class DoNeRFTRT(nn.Module):
    def __init__(self, D, W, skip, n_in, n_out):
        super(DoNeRFTRT, self).__init__()
        self.net_idx = 1
        if "auto" in skip:
            skip_layer = 7
            if len(skip) > 4:
