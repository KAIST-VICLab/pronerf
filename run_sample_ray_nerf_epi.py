...
def render_rays(ray_batch, or_ray_batch, ref_poses, ref_rgbs, Hfull, Wfull, ref_K, k_ref,
                network_fn,
                network_query_fn,
                N_samples,
                retraw=False,
    ...
    if randomize is True:
        ...
        if random.random() > 0.5:
            epi_z_vals_diff = torch.abs(
                epi_z_vals - torch.cat((epi_z_vals[:, 1:], or_far * torch.ones(H * W, 1).type_as(epi_z_vals)), 1))
            noise_ = noise_ * epi_z_vals_diff
        else:
            epi_z_vals_diff = torch.abs(
                epi_z_vals - torch.cat((or_near * torch.ones(H * W, 1), epi_z_vals[:, 0:-1].type_as(epi_z_vals)), 1))
            noise_ = -noise_ * epi_z_vals_diff
    ...
    depths = epi_z_vals.view(1, 1, N_rays, N_samples).repeat(k_ref, 1, 1, 1) # k_ref, 1, N_rays, N_samples
    depths = torch.permute(depths, (0, 3, 1, 2)).reshape(-1, 1, N_rays)  # k_ref * N_samples, 1, N_rays
    ...
    warps_flat = torch.permute(warps_flat.view(k_ref, N_samples, 3, N_rays), (3, 1, 0, 2)).reshape(N_rays, N_samples, 3*k_ref)
    ...
    ret = {'rgb_map0': rgb0, 'rgb_map1': rgb_map, 'depth_map':mean_epi_depth, 'mean_warp':depth_map, 'acc_map':acc_map}
...
#            **kwargs):
def render(H, W, K, ref_K, k_ref, rays=None, c2w=None, ndc=True, ref_poses=None, ref_rgbs=None,
           near=0., far=1., or_near=1., or_far=10., use_viewdirs=False, c2w_staticcam=None, **kwargs):
    """Render rays
    Args:
    ...
    # Render and reshape
    all_ret = render_rays(rays, or_rays, ref_poses, ref_rgbs, H, W, ref_K, k_ref, **kwargs)
    for k in all_ret:
...
def config_parser():
    ...
    parser.add_argument("--double_test_sample", action='store_true',
                        help='embed ray point encoding')
    parser.add_argument("--det_sampling", action='store_true',
                        help='deterministic sampling after 200,000 iterations')
    parser.add_argument("--weight_decay", type=float, default=0.0,
                        help='regularization on weights')
    parser.add_argument("--disp_smoooth", type=float, default=0.0,
                        help='disp_smooth_loss')
...
def train():
    ...
    if use_batching:
        ...
        ray_dir_diff = rays_rgb[0, 1, :] - rays_rgb[1, 1, :] # [3]
        ray_dir_diff = torch.Tensor(ray_dir_diff).to(device)
        print('shuffle rays')
    ...
    # Get ordered rays for epipolar lines
    poses_train = torch.stack([poses[i] for i in i_train], 0)
    ref_images = torch.stack([images[i] for i in i_train], 0)
    ...
    for i in trange(start, N_iters):
        ...
        rgb0, rgb1, depth, mean_warp, extras = render(H, W, K, ref_K, args.k_ref, rays=batch_rays, ref_poses=ref_poses,
                                                      ref_rgbs=ref_rgbs, verbose=i < 10,
        ...
        mm_rgb_loss = 0
        if args.a_mmrgb > 0:
            mm_rgb_loss = args.a_mmrgb * img2mse(rgb0, target_s)
        ...
        loss = img_loss + mm_rgb_loss + acc_reg
