...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, default='configs/lego/lego_epinerf_refine_SeSeN.txt',
...
def render_path(render_poses, hwf, K, k_ref, N_n, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    get_acc_map = render_kwargs.get('get_acc_map', False)
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if get_acc_map:
            acc_map = rgbs1[-1].mean(-1, keepdims=True)
            acc_map[acc_map < 0.99] = 0.0
...
def render_rays(ray_batch, or_ray_batch, p_uv, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
    ...
    pts, z_vals = compute_query_points_from_rays(rays_o, rays_d, near, far, N_point_ray_enc, False)
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if  (not args.render_test) and (not args.render_only):
            render_kwargs_train['train_nerf'] = True
            render_kwargs_train['train_sampler'] = True
        ...
        #     # for mask only
        #     if args.render_only:
        #         testsavedir = os.path.join(args.datadir, 'mask')
        #         os.makedirs(testsavedir, exist_ok=True)
        ...
        #     with torch.no_grad():
        #         render_kwargs_test['get_acc_map'] = True
        #         r_out = render_path(render_poses, hwf, K, args.k_ref, args.N_n, args.chunk, ref_poses=poses_train, p_uv=the_p_uv.view(-1, 2),
        #                     ref_rgbs=the_ref_rgb, render_kwargs=render_kwargs_test,
