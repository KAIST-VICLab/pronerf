...
def batchify_rays(rays_flat, or_rays_flat, chunk=1024 * 32, **kwargs):
    ...
    batch_pos2d = kwargs['batch_pos2d']
    for i in range(0, rays_flat.shape[0], chunk):
        kwargs['batch_rays_nearest_id'] = batch_rays_nearest_id[i:i + chunk]
        kwargs['batch_pos2d'] = batch_pos2d[i:i + chunk]
...
def render_path(render_poses, hwf, K, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        width_step = np.linspace(0, 1, num=W)
        height_step = np.linspace(0, 1, num=H)
        x,y = np.meshgrid(width_step, height_step)
        pos_2d = np.stack([x,y, np.ones_like(x)], axis = -1)
        render_kwargs['batch_pos2d'] = torch.Tensor(pos_2d).view(-1,3).to(device)
...
def create_nerf(args):
    ...
    model_refine = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                ...
                                6 * (0+args.N_samples) + 3 * args.num_neighbor * args.N_samples + input_ch,
                                output_ch=args.N_samples + 3 + 3, skips=args.mmnetskips)
    s_grad_vars.append({'params': model_refine.parameters(), 'weight_decay': 5e-8, 'lr': args.lrate})
...
def render_rays(ray_batch, or_ray_batch,
    ...
    pos2d_feat = embed_fn(kwargs.get('batch_pos2d'))
    net_input = torch.cat((plucker_embed.view(N_rays, -1), epi_features.view(N_rays, -1), pos2d_feat), -1)
    ...
    refine_rgb = torch.sigmoid(refine_output[:, N_samples:N_samples+3])
    points_offset = torch.tanh(refine_output[:, N_samples+3:])[:,None,:].repeat(1, N_samples, 1)
    ...
    if train_sampler: # and iter >= 30000:
        ...
        scale_factor = ((query_points_nerf[..., -1] + 1) / 2)**(2)
        query_points_nerf = query_points_nerf + \
            (1e-2) * points_offset * scale_factor[..., None]
...
def train():
    ...
    if use_batching:
        ...
        x,y = np.meshgrid(width_step, height_step)
        pos_2d = np.stack([x,y, np.ones_like(x)], axis = -1)
        pos_2d = pos_2d[None,:,:,None,:].repeat(rays_rgb.shape[0],axis=0)
        rays_rgb = np.concatenate([rays_rgb, pos_2d], axis=3)

        rays_rgb = np.reshape(rays_rgb, [-1, 4, 3])  # [(N-1)*H*W, ro+rd+rgb + pixel2d, 3]
    ...
    for i in trange(start, N_iters):
        ...
        batch_rays, target_s, batch_pos2d = batch[:2], batch[2], batch[3]
        ...
        render_kwargs_train['batch_pos2d'] = batch_pos2d
