...
def config_parser():
    ...
    parser.add_argument("--lrate", type=float, default=5e-4,
                        help='learning rate')
    parser.add_argument("--ngp_lrate", type=float, default=1e-3,
                        help='ngp learning rate')
...
def run_network(inputs, viewdirs, density_fn, color_fn, embed_fn, embeddirs_fn, netchunk=1024 * 64, res_feat = None):
    """Prepares inputs and applies network 'fn'.
    """
    geo_feat_dim = 15
    positions = torch.reshape(inputs, [-1, inputs.shape[-1]])
    directions = viewdirs[:, None].expand(-1, inputs.shape[1], -1).reshape(-1, 3)

    aabb = torch.tensor([[-1.5, -1.5, -1.5], [1.5, 1.5, 1.5]], dtype=torch.float32)
    norm_positions = SceneBox.get_normalized_positions(positions, aabb)

    embedded = embed_fn(norm_positions) # has position embed
    embedded_dirs = embeddirs_fn(directions) # dir embed
    
    if res_feat is not None:
        embedded = embedded + res_feat.reshape(-1, embedded.shape[1])

    density_outputs = batchify(density_fn, netchunk)(embedded)
    raw_density, geo_feat = torch.split(density_outputs, [1, geo_feat_dim], dim=-1)
    ...
    raw_color = batchify(color_fn, netchunk)(h)
    raw = torch.cat([raw_color, raw_density], dim =-1)

    outputs = torch.reshape(raw, list(inputs.shape[:-1]) + [raw.shape[-1]])
    return outputs


def batchify_rays(rays_flat, or_rays, target_pose, ref_poses, ref_rgbs,
                          H, W, K, ref_K, k_ref, N_n, chunk=1024*32, **kwargs):
    """Render rays in smaller minibatches to avoid OOM.
    """
    all_ret = {}
    for i in range(0, rays_flat.shape[0], chunk):
        ret = render_rays(rays_flat[i:i+chunk], or_rays[i:i+chunk], target_pose, ref_poses,
...
def render(H, W, K, ref_K, k_ref, N_n, coords=None, chunk=1024 * 32, rays=None, c2w=None, ndc=True,
           target_pose=None, ref_poses=None, ref_rgbs=None,
           near=0., far=1., or_near=2., or_far=6.,
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, or_rays, target_pose, ref_poses, ref_rgbs,
                          H, W, K, ref_K, k_ref, N_n, chunk, **kwargs)
    for k in all_ret:
...
def render_path(render_poses, hwf, K, k_ref, N_n, chunk, ref_poses, ref_rgbs, render_kwargs,
    ...
    render_kwargs['train_sampler'] = False

    t = time.time()
    for i, c2w in enumerate(tqdm(render_poses)):
        t = time.time()
        rgb0, rgb1, depth, nerf_depth, extras = render(H, W, K, torch.Tensor(K.copy()).to(device), k_ref, N_n,
        ...
        if savedir is not None:
            ...
            rgb8 = to8b(rgbs_n0[-1])
            filename = os.path.join(savedir, '{:03d}_rgbn0.png'.format(i))
            imageio.imwrite(filename, rgb8)
...
def create_nerf(args):
    ...
    grad_vars.append({'params': embed_fn.parameters(), 'eps': 1e-15, 'lr': args.ngp_lrate})
    input_ch = embed_fn.get_out_dim()
    
    input_ch_views = 0
    embeddirs_fn = None
    if args.use_viewdirs:
        embeddirs_fn = SHEncoding(
            levels=4,
        )
        input_ch_views = embeddirs_fn.get_out_dim()

    geo_feat_dim = 15

    model_density = TCNNMLP(
        in_dim=input_ch,
        num_layers=2,
        layer_width=64,
    ...
    grad_vars.append({'params': model_density.parameters(),'lr':  args.ngp_lrate})
    ...
    grad_vars.append({'params': model_color.parameters(), 'lr':  args.ngp_lrate})

    model_fine = None
    network_query_fn = lambda inputs, viewdirs, density_fn, color_fn, res_feat: run_network(inputs, viewdirs, density_fn, color_fn,
                                                                        embed_fn=embed_fn,
                                                                        embeddirs_fn=embeddirs_fn,
                                                                        netchunk=args.netchunk, res_feat=res_feat)

    s_grad_vars = []

    model_mmray = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                input_ch=input_ch * args.N_point_ray_enc if args.mm_emb else
                                 3 * args.N_point_ray_enc,
                                output_ch=3 * args.N_samples + 3, skips=args.mmnetskips)
    # model_mmray = TCNNMLP(num_layers=args.mmnetdepth, layer_width=args.mmnetwidth,
    #                             in_dim=input_ch * args.N_point_ray_enc if args.mm_emb else
    #                              3 * args.N_point_ray_enc,
    #                             out_dim=3 * args.N_samples + 3, activation=nn.ReLU(), out_activation=None)
    ...
    model_refine = MinMaxRay_Net(D=args.mmnetdepth, W=args.mmnetwidth,
                                input_ch=input_ch * args.N_samples + 3 * args.N_n * args.N_samples if args.mm_emb else
                                  3 * args.N_samples + 3 * args.N_n * args.N_samples,
                                output_ch=4 * args.N_samples + 3 + input_ch * args.N_samples, skips=args.mmnetskips)
    # model_refine = TCNNMLP(num_layers=args.mmnetdepth, layer_width=args.mmnetwidth,
    #                             in_dim=input_ch * args.N_samples + 3 * args.N_n * args.N_samples if args.mm_emb else
    #                              3 * args.N_samples + 3 * args.N_n * args.N_samples,
    #                             out_dim=4 * args.N_samples + 3 + input_ch * args.N_samples, activation=nn.ReLU(), out_activation=None)


    s_grad_vars.append({'params': model_refine.parameters(), 'weight_decay': args.weight_decay, 'lr': args.lrate})

    # Create optimizer
    optimizer = torch.optim.RAdam(params = grad_vars, lr= args.ngp_lrate, betas=(0.9, 0.99))
    ...
    if len(ckpts) > 0 and not args.no_reload:
        ...
        # Load model
        model_density.load_state_dict(ckpt['density_fn_state_dict'])
        model_color.load_state_dict(ckpt['color_fn_state_dict'])
    ...
    render_kwargs_train = {
        ...
        'density_fn': model_density,
        'color_fn': model_color,
...
def raw2outputs(raw, z_vals, rays_d, raw_noise_std=0, white_bkgd=False, pytest=False, mm_density_add=None,
    ...
    if mm_density_add is not None:
        ...
        raw_density = raw[..., 3]
        raw_density = torch.clamp(raw_density, min= -1e3, max=1e3)
...
def render_rays(ray_batch, or_ray_batch, target_pose, ref_poses, ref_rgbs, Hfull, Wfull, K, ref_K,
                k_ref, N_n,
                density_fn,
                color_fn,
                network_query_fn,
    ...
    # compute spherical intersection
    spherical_near, spherical_far = intersect_sphere(rays_o, rays_d)
    near = spherical_near
    far = spherical_far
    ...
    with torch.no_grad():
        ...
        rel_cam_idx = rel_cam_idx[0:N_n*2]
        if randomize:
            # Random but keep order
            ref_nos = rel_cam_idx.int().tolist()
            order_idx = sorted(random.sample(range(len(ref_nos)), N_n))
            ref_nos = [ref_nos[i] for i in order_idx]
        else:
            # Nearest
            ref_nos = rel_cam_idx[0:N_n].int().tolist()

        C_color = ref_rgbs.shape[-1]
        ref_rgb = ref_rgbs[ref_nos].view(N_n, Hfull, Wfull, C_color)
        ...
        ro1, rd1 = ro1.repeat(N_point_ray_enc * N_n, 1, 1), rd1.repeat(N_point_ray_enc * N_n, 1, 1)
        ref_K_ = ref_K.unsqueeze(0).repeat(N_point_ray_enc * N_n, 1, 1)
        inv_K = torch.inverse(ref_K_)

    # This code no grad???
    depths = depth_values.view(1, 1, N_rays, N_point_ray_enc).repeat(N_n, 1, 1, 1) # k_ref, H, W, N_point_ray_enc
    depths = torch.permute(depths, (0, 3, 1, 2)).reshape(-1, 1, N_rays)  # k_ref * N_point_ray_enc, H, W

    warps, npcoords = inverse_warp.inverse_warp_rod1_rt2_coords(ref_rgb, depths, ro1, rd1, ref_pose, ref_K_, inv_K,
                                                    padding_mode='zeros')
    warps_flat = warps.clone().view(1, N_n, N_point_ray_enc, C_color, 1, N_rays)

    # Make invalid regions to the mean of samples
    valid_warp = (torch.sum(warps_flat.detach(), 3, True) > 0).type_as(warps).repeat(1, 1, 1, C_color, 1, 1)
    ...
    # net_input = torch.cat((plucker_pts, warps_flat.view(N_rays, -1)), -1)
    net_input = torch.cat((epi_pts.view(N_rays, -1), warps_flat.view(N_rays, -1)), -1)
    ...
    refine_rgb = torch.sigmoid(refine_output[:, 4 * N_samples:4 * N_samples+3])

    if randomize and not train_sampler:
        res_feat = None
    else:
        res_feat = refine_output[:,4 * N_samples+3:].view(N_rays, N_samples, -1)
        # res_feat = None
    ...
    if randomize and not train_sampler:
        max_mult = 8
        n_mult = random.randint(2, max_mult)
        ...
        if n_mult > 1:
            ...
            epi_z_vals_diff = torch.abs(
                refine_depth_values -
                torch.cat((refine_depth_values[:, 1:], far), 1))
            upper_noise_ = mults[:, None, :] * epi_z_vals_diff[:, :, None]  # N_Rays, N_samples, n_mult

            epi_z_vals_diff = torch.abs(
                refine_depth_values -
                torch.cat((near, refine_depth_values[:, 0:-1].type_as(refine_depth_values)), 1))
            lower_noise_ = -mults[:, None, :] * epi_z_vals_diff[:, :, None]
            
            noise_ = torch.cat([upper_noise_, lower_noise_], dim = -1)
            epi_z_vals = refine_depth_values[:, :, None] + noise_
            epi_z_vals = epi_z_vals.view(N_rays, 2* N_samples * n_mult)
            epi_z_vals, _ = torch.sort(epi_z_vals, dim=-1) # not really needed?
            N_samples = 2 * n_mult * N_samples
        else:
    ...
    query_points_nerf = rays_o[..., None, :] + rays_d[..., None, :] * epi_z_vals[..., :, None]
    raw = network_query_fn(query_points_nerf, viewdirs, density_fn, color_fn, res_feat)

    if randomize and not train_sampler:
        rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std,
        ...
        # max_noise = 0.99
        # min_noise = 0.01
        # noise_ = (1 / N_samples) * torch.normal(0.0, 1.0, size=noise_shape).type_as(epi_z_vals)
        ...
        # upper_noise_ = noise_ * epi_z_vals_diff # N_Rays, N_samples

        # epi_z_vals_diff = torch.abs(
        #     epi_z_vals - torch.cat((near * torch.ones(N_rays, 1), epi_z_vals[:, 0:-1].type_as(epi_z_vals)), 1))
        # lower_noise_ = -noise_ * epi_z_vals_diff

        # noise_ = torch.stack([lower_noise_, torch.zeros_like(lower_noise_), upper_noise_], dim =-1)
        # explore_z_vals = epi_z_vals[..., None].clone().detach() + noise_

        # explore_points = rays_o[..., None, :] + rays_d[..., None, :] * explore_z_vals.view(N_rays, -1)[..., :, None]

        # with torch.no_grad():
        #     explore_raw = network_query_fn(explore_points, viewdirs, density_fn, color_fn, None)
        #     explore_raw = explore_raw.view(N_rays, N_samples, 3, -1)
        #     explore_density = explore_raw[...,3]
        #     softmax_density = F.softmax(explore_density, dim =-1)
        #     mean_explore_z_vals = (explore_z_vals*softmax_density).sum(-1)

        # explore_loss = ((refine_depth_values - mean_explore_z_vals.detach())**2).mean(-1, keepdim =True)
    else:
        raw_noise_std = 0
        rgb_map, disp_map, acc_map, weights, depth_map = raw2outputs(raw, epi_z_vals, rays_d, raw_noise_std,
                                                                     white_bkgd, pytest=pytest)
...
def train():
    ...
    alpha_masks = torch.Tensor(alpha_masks).to(device)[i_train]
    ...
    # blur_ref_rgbs = torch.permute(rays_rgb[:, :, :, 2, :].clone(), (0, 3, 1, 2))
    # blur_ref_rgbs = F.avg_pool2d(blur_ref_rgbs, kernel_size=5, padding=2, stride=1, count_include_pad=True)
    # blur_ref_rgbs = torch.permute(blur_ref_rgbs, (0, 2, 3, 1))
    ...
    #         r_out = render_path(render_poses, hwf, K, args.k_ref, args.N_n, args.chunk, ref_poses=poses_train,
    #                             ref_rgbs=the_ref_rgb, render_kwargs=render_kwargs_test)
    #         rgbs0, rgbs1, depths, nerf_depths, rgbsn0 = r_out[0], r_out[1], r_out[2], r_out[3], r_out[4]
    #         print('Done rendering', testsavedir)
    #         moviebase = os.path.join(testsavedir, '{}_spiral_'.format(expname))
    #         imageio.mimwrite(moviebase + 'rgbn0.mp4', to8b(rgbsn0), fps=30, quality=8)
    ...
    #         imageio.mimwrite(moviebase + 'nerf_depth.mp4', to8b(nerf_depths / np.percentile(nerf_depths, 99)), fps=30, quality=8)
    ...
    all_the_coords = range(H * W)
    # center_crop = np.zeros((H, W))
    # center_crop[H//4:3*H//4, W//4:3*W//4] = 1
    # center_crop = list(center_crop.flatten())
    # center_coords = [this_coord for (n, this_coord) in enumerate(all_the_coords) if center_crop[n] == 1]

    dilated_achannel = F.max_pool2d(alpha_channel[:, None], kernel_size=9, padding=4, stride=1)
    vect_achann = dilated_achannel.view(-1, H * W)
    all_valid_coords = []
    for n_coords in range(rays_rgb.shape[0]):
        all_valid_coords.append(list(torch.tensor(all_the_coords)[vect_achann[n_coords] > 0].cpu().numpy()))
    ...
    for i in trange(start, N_iters):
        with torch.no_grad():
            ...
            ref_rgbs = [the_ref_rgb[n_ref] for n_ref in range(rays_rgb.shape[0]) if n_ref != img_i]
            ...
            a_chann = alpha_channel[img_i, :, :]
            pose = poses_train[img_i, :3, :4]

            # Random selection of rays (once in a while sample some invalid rays)
            if i > args.precrop_iters and i % 11 == 0:
                all_coords = all_the_coords
            else:
                all_coords = all_valid_coords[img_i]
            select_inds = random.sample(all_coords, N_rand)
            # p_uv = the_p_uv.view(H * W, 2)[select_inds]
            rays_o = rays_o.view(H * W, 3)[select_inds]  # (N_rand, 3)
            rays_d = rays_d.view(H * W, 3)[select_inds]  # (N_rand, 3)
            batch_rays = torch.stack([rays_o, rays_d], 0)
            target_s = target.view(H * W, 3)[select_inds]  # (N_rand, 3)
            a_chann = a_chann.view(H * W)[select_inds]
            ref_rgbs = ref_rgbs.view(-1, H * W, c_color)  # (N_ref, H*W, 3)
        ...
        if i % 3 != 0:
            ...
            rgb0, rgb1, depth, nerf_depth, extras = render(H, W, K, ref_K, args.k_ref, args.N_n,
            ...
            optimizer.zero_grad()
            loss = img2mse(rgb1, target_s)
            psnr_n = mse2psnr(img2mse(rgb1.detach(), target_s))
            loss.backward()
            ...
            loss = img2mse(rgb1, target_s) + img2mse(rgb0, target_s) + img2mse(extras['mm_rgb'], target_s) 
            
            # if i >= 30000:
            #     loss = loss + extras['explore_loss'].mean()
            psnr_sn = mse2psnr(img2mse(rgb1.detach(), target_s))
            loss.backward()
            if (not torch.isnan(loss)) and (not torch.isinf(loss)) :
                s_optimizer.step()
                # optimizer.step()
        ...
        if i % args.i_weights == 0 and (not args.render_test):
            ...
            torch.save({
                ...
                'density_fn_state_dict': render_kwargs_train['density_fn'].state_dict(),
                'color_fn_state_dict': render_kwargs_train['color_fn'].state_dict(),
        ...
        if i % args.i_print == 0:
            if psnr_n != 0:
                psnr_n = psnr_n.item()
            if psnr_sn != 0:
                psnr_sn = psnr_sn.item()
            tqdm.write(f"[TRAIN] Iter: {i} Loss: {loss.item()}  PSNR_n: {psnr_n}  PSNR_sn: {psnr_sn}")
